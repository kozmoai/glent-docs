...
Glint's API provides tools to easily query, search and filter software catalog data.

## Common queries usage

High quality search is essential to effectively track assets in your software catalog, using Glint's search you can:
...
The base search route is `https://api.useglint.io/v1/entities/search`, it receives HTTP POST requests.

A search request defines the logical relation between different search rules, and contains filters and rules to find matching <GlintTooltip id="entity">entities</GlintTooltip>.
...
Glint has 2 types of search rule operators:
...
- The `propertySchema` can be used with any Glint [property](/build-your-software-catalog/customize-integrations/configure-data-model/setup-blueprint/properties/properties.md#supported-properties);
...
![Dependency graph upstream downstream diagram](/img/software-catalog/search-in-glint/search-direction-diagram.png)
  ...
  "entities": [
    {
      ...
      "blueprint": "deploymentConfig",
      "properties": {
        "url": "https://github.com/kozmoai/order-service",
        "config": {
          "encryption": "SHA256"
        },
        "monitor-links": [
    ...
    },
    {
      ...
      "blueprint": "deploymentConfig",
      "properties": {
        "url": "https://github.com/kozmoai/cart-service",
        "config": {
          "foo": "bar"
        },
        "monitor-links": [
...
When using Glint's UI, you can use properties of the logged-in user when writing rules by using the following functions:
