...
Glint then allows engineers to perform actions on these assets in a self-service fashion. From provisioning a dev environment, understanding who is the owner of a microservice, or any unique use case DevOps want to self-serve and automate.
...
Glint's three core building blocks are _Blueprints_, _Entities_ and _Relations_. This tutorial will walk you through your first steps on the platform and get you started on your Developer Portal journey!🚢

## The goal of this tutorial

The goal of this tutorial is:

- Teach you about Glint's core components;
- Familiarize you with Glint's web UI;
- View functional code snippets to interact with Glint's API;
- Experience the power of Glint as an internal developer platform.

This guide will give you a foundation to start building your software catalog in Glint and view a complete image of your software infrastructure.
...
Blueprints are used to model data in Glint. A Blueprint allows us to define what properties and fields an _Entity_ will contain.

Architectures and deployments vary greatly, and so do preferences and standards for data representation and asset structure. Therefore, in Glint, You have full control of the way data is presented using any data format you desire, so that the Software Catalog truly represents all you need for the developer portal.
...
:::tip
In this tutorial, we will demonstrate how to perform every step using Glint's web UI and Glint's REST API.

This tutorial includes various examples of ways to interact with Glint's API. For more, you are welcome to visit the API section in Blueprint basics.

Glint also has a GitHub app and a Terraform provider you can use to ingest data and interact with Glint's components.
...
Let's head to [Glint](https://app.useglint.io/dev-portal) and look at the DevPortal Builder page, at the top right corner let's click on **Add Blueprint** and configure our first Blueprint - **Service** as shown in the image below:
...
To interact with Glint's API, we will use python, the only package requirement is the [requests](https://pypi.org/project/requests/) library which you can install by running:
...
:::note
For the next part, you will need your Glint `CLIENT_ID` and `CLIENT_SECRET`.

To find your Glint API credentials go to [Glint](https://app.useglint.io), hover on the `3 dots button` at the top right corner, select `Credentials` and then you will be able to view and copy your `CLIENT_ID` and `CLIENT_SECRET`:
...
In order to perform any action with Glint's API, you first need an **access token**:
...
Now that you have an access token, you can use it for every interaction you make with Glint's API. You will also use it in the section below to create the `Service` Blueprint:
...
{
  "properties": {
    "on-call": "mor@useglint.io",
    "language": "Python",
    "number-of-open-jira-issues": 21,
    "product": "Analytics",
    "url": "https://github.com/glint/notification-service",
    "config": {
...
To conclude your first steps with Glint, we use Blueprints to define our data model, and Entities to store data objects that match the type of our Blueprints.
...
Feel free to continue exploring the specific Entity page and the environments, services and running service pages. Notice the `filter`, `hide`, `sort` and `group by` controls you can find at the top right of Glint's table widgets.

You can also use Glint's API to make GET requests for Blueprints and Entities, here is a code example to get the running service Entity we created in this tutorial:
...
In addition, you can also use Glint's API to search for Blueprints and Entities.

## What now?

Congratulations! you just modeled your first environment in Glint! 🎉🚢

This quickstart was used to teach you the basic building blocks Glint provides. Now, you have all the tools you need to start cataloging and tracking your environment!
...
Remember that the blueprints, entities and relations you created here were used as a basic example, but Glint always allows you to go back and edit them until they match the infrastructure you want to catalog.
...
:::tip
These suggestions show the basic steps in creating your very own Developer Portal, if you want to learn more about Glint before starting your Developer Portal journey, look at [Diving deeper](#diving-deeper) or [Using the API](#using-the-api) below.
:::

1. Create Blueprints for your software and infrastructure components;
2. Map out the Relations between your Blueprints;
3. Ingest data to your catalog by creating Entities based on your Blueprints via Glint's UI or using our API;
...
If you want to learn more about Glint's capabilities in a specific area, you can check out any of these resources:
...
If you want to continue utilizing Glint's REST API, take a look at these resources:
