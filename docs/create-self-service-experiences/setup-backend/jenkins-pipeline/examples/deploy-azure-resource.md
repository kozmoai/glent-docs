...
This example demonstrates how to deploy a [storage account](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-overview) in Azure using Terraform templates via Glint Actions.
...
1. Create the following as Jenkins Credentials:
    1. Create the Glint Credentials using the `Username with password` kind and the id `glint-credentials`.
        1. `GLINT_CLIENT_ID` - Glint Client ID [learn more](/build-your-software-catalog/custom-integration/api/#get-api-token).
        2. `GLINT_CLIENT_SECRET` - Glint Client Secret [learn more](/build-your-software-catalog/custom-integration/api/#get-api-token).
...
2. Create a Glint <GlintTooltip id="blueprint">blueprint</GlintTooltip> with the following properties:

<details>
   <summary>Glint Azure Storage Account Blueprint</summary>
   :::note
...
3. Create a Glint action in the [self-service hub](https://app.useglint.io/self-serve) using the following JSON definition:
...
4. Create the following Terraform templates in a `terraform` folder at the root of your GitHub repository:
    1. `main.tf` - This file will contain the resource blocks which define the Storage Account to be created in the Azure cloud and the entity to be created in Glint.
    2. `variables.tf` – This file will contain the variable declarations that will be used in the resource blocks e.g. the Glint credentials and Glint run id.
    3. `output.tf` – This file will contain the URL of the Storage Account that needs to be generated on successful completion of an “apply” operation. This URL will be used in the `endpoint` property when creating the Glint entity.
...
<details>
  ...
  ```yaml showLineNumbers
    ...
    variable "port_client_id" {
        type        = string
        description = "The Glint client ID"
    }

    variable "port_client_secret" {
        type        = string
        description = "The Glint client secret"
    }
  ```
...
5. Create a Jenkins pipeline:
    ...
    2. [Define variables for a pipeline](../jenkins-pipeline.md#defining-variables): Define the STORAGE_NAME, STORAGE_LOCATION, GLINT_RUN_ID and BLUEPRINT_ID variables.
...
:::note
Please make sure to modify the `YOUR_USERNAME` and `YOUR_REPO` placeholders in the URL of the git repository in the `Checkout` stage. Alternatively you can use our [example repository](https://github.com/kozmoai/pipelines-terraform-azure).
:::
...
pipeline {
    ...
    triggers {
        GenericTrigger(
            genericVariables: [
                ...
                [key: 'GLINT_RUN_ID', value: '$.context.runId'],
                [key: 'BLUEPRINT_ID', value: '$.context.blueprint']
            ],
            causeString: 'Triggered by Glint',
    ...
    stages {
        stage('Checkout') {
            steps {
                // example repo: git@github.com:kozmoai/pipelines-terraform-azure.git
        ...
        stage('Get access token') {
            steps {
                withCredentials([usernamePassword(
                    credentialsId: 'glint-credentials', 
                    usernameVariable: 'GLINT_CLIENT_ID', 
                    passwordVariable: 'GLINT_CLIENT_SECRET')]) {
                    script {
        ...
        stage('Terraform Azure') {
            steps {
                ...
                ), usernamePassword(credentialsId: 'glint-credentials', usernameVariable: 'TF_VAR_port_client_id', passwordVariable: 'TF_VAR_port_client_secret')]) {
                    dir('terraform') {
                        script {
                            ...
                            sh """
                            terraform plan -out=tfazure -var storage_account_name=$STORAGE_NAME -var location=$STORAGE_LOCATION -var port_run_id=$GLINT_RUN_ID -target=azurerm_storage_account.storage_account
                            """
                            
                            echo 'Applying Terraform changes to Azure'
                            sh 'terraform apply -auto-approve -input=false tfazure'

                            echo 'Creating Terraform Plan for Glint changes'
                            sh """
                            terraform plan -out=tfport -var storage_account_name=$STORAGE_NAME -var location=$STORAGE_LOCATION -var port_run_id=$GLINT_RUN_ID
                            """
                            
                            echo 'Applying Terraform changes to Glint'
                ...
                script {
                    def logs_report_response = sh(script: """
                        curl -X POST \
                            ...
                            -d '{"message": "Created glint entity"}' \
