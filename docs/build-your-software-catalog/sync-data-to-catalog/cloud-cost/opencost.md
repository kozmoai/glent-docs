...
Our OpenCost integration allows you to import `cost` from your OpenCost instance into Glint, according to your mapping and definition.
...
helm upgrade --install my-opencost-integration kozmoai/glint-ocean \
...
The integration configuration determines which resources will be queried from OpenCost, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the OpenCost object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: cost
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one OpenCost object to a Glint entity. Each value is a JQ query.
            identifier: .name
            title: .name
            blueprint: '"openCostResourceAllocation"'
            properties:
