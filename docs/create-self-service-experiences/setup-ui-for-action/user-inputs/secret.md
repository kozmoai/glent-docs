...
Secret input is an input type used to pass secrets and sensitive information to action backends, the values sent via the secret input go through an additional layer of encryption using your private key. In addition, the values sent via the secret input are not logged or saved by Glint.
...
- [aes256-gcm](https://www.nist.gov/publications/advanced-encryption-standard-aes) - This will encrypt the property data using 256 bits AES in [GCM mode](https://csrc.nist.gov/glossary/term/aes_gcm). The encrypted value will be prefixed by the 16 bits IV and suffixed by the 16 bits MAC, encoded to base-64. The encryption key will be the first 32 bytes of your organization's [Client Secret](../../../build-your-software-catalog/custom-integration/api/#find-your-glint-credentials).
...
def webhook():
    ...
    key = GLINT_CLIENT_SECRET[:32].encode()
...
if __name__ == '__main__':
    glint = int(os.getenv('GLINT', 80))

    print("Starting app on glint %d" % glint)

    app.run(debug=False, glint=glint, host='0.0.0.0')
```
...
const GLINT_CLIENT_SECRET = "YOUR GLINT CLIENT SECRET";
...
app.post("/", bodyParser.json(), (req, res) => {
  ...
  const encodedSecret = Buffer.from(GLINT_CLIENT_SECRET.substring(0, 32));
