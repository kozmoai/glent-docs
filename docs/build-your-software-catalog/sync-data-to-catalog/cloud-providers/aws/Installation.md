...
- You will need your [Glint credentials](/build-your-software-catalog/custom-integration/api/api.md#find-your-glint-credentials) to install the AWS exporter:
...
# Export your Glint credentials
export GLINT_CLIENT_ID=YOUR-GLINT-CLIENT-ID
export GLINT_CLIENT_SECRET=YOUR-GLINT-CLIENT-SECRET

# Clone the terraform template
git clone https://github.com/kozmoai/template-assets.git
...
1. Creates the resource blueprints in your Glint environment.
2. Deploys the AWS exporter in your AWS environment with the following resources - [S3 bucket](https://aws.amazon.com/s3/), [mapping configuration file](/build-your-software-catalog/sync-data-to-catalog/cloud-providers/aws#exporter-configjson-file), [AWS secret](./aws.md#glint-credentials-secret), [AWS IAM policy](./aws.md#iam-policy);
...
- **Secret related parameters:**
  ...
  - `CustomGlintCredentialsSecretARN` - The ARN of the Glint credentials secret;

    **OR**

  - `SecretName` - The name for the new Glint credentials secret to create.
...
1. Prepare a [`config.json`](/build-your-software-catalog/sync-data-to-catalog/cloud-providers/aws#exporter-configjson-file) file that will define which AWS resources to ingest to Glint;
...
3. Deploy our [`serverless application`](#exporter-aws-serverless-application).
   ...
   You can deploy the application from the AWS console through this [link](https://eu-west-1.console.aws.amazon.com/lambda/home?region=eu-west-1#/create/app?applicationId=arn:aws:serverlessrepo:eu-west-1:185657066287:applications/glint-aws-exporter).
   ...
   1. Create a `parameters.json` file to override certain parameters values. For example (replace the placeholders):
      ...
      [
        ...
        {
          "Name": "CustomGlintCredentialsSecretARN",
          "Value": "<YOUR_GLINT_CREDENTIALS_SECRET_ARN>"
        },
        {
          "Name": "CreateBucket",
          "Value": "false"
   ...
   2. Use the following command to create a change set:
      ...
      ```bash showLineNumbers
      aws serverlessrepo create-cloud-formation-change-set --application-id arn:aws:serverlessrepo:eu-west-1:185657066287:applications/glint-aws-exporter --stack-name glint-aws-exporter --capabilities CAPABILITY_IAM CAPABILITY_RESOURCE_POLICY --parameter-overrides file://parameters.json

      # Result
      {
        "ApplicationId": "arn:aws:serverlessrepo:eu-west-1:185657066287:applications/glint-aws-exporter",
...
```bash showLineNumbers
sam list stack-outputs --stack-name serverlessrepo-glint-aws-exporter
```

The list includes:

- `Lambda Function ARN` - the ARN of the exporter's Lambda;
- `Glint Credentials Secret ARN` - the ARN of the Glint credentials secret;
...
4. Update the [`Glint credentials secret`](#glint-credentials-secret) with your credentials;
...
```bash showLineNumbers
sam logs --stack-name serverlessrepo-glint-aws-exporter --tail
```
...
2. Deploy the event rule using this command:
   ...
   ```bash showLineNumbers
   aws cloudformation deploy --template-file template.yml --stack-name glint-aws-exporter-event-rules
   ```
