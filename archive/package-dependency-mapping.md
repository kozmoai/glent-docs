...
Using Glint, it is very easy to implement and track package dependency mapping for your microservices. That can be useful, for example, while trying to investigate a dependency related issue or bug, or while checking for security issues.

:::tip
All relevant files and resources for this guide are available [**HERE**](https://github.com/kozmoai/demo-node-project) for the `yarn.lock` use-case, and [**HERE**](https://github.com/kozmoai/demo-node-poject-npm) for the `package-lock.json` use-case.
:::
...
Here is a [link](https://github.com/kozmoai/demo-node-project) to the Git repository we will be working on in this use-case.
...
[This Python script](https://github.com/kozmoai/demo-node-project/blob/main/.github/scripts/scan-yarn-lock.py) has some useful functions with which we interact with Glint.
Have a look at:
...
# Glint Client Secret and ID
CLIENT_ID = os.environ.get("GLINT_CLIENT_ID")
CLIENT_SECRET = os.environ.get("GLINT_CLIENT_SECRET")
...
Let's walk through the [workflow](https://github.com/kozmoai/demo-node-project/blob/main/.github/workflows/update-packages.yml).

```yaml showLineNumbers
name: Update Packages in Glint
...
on:
  ...
  # Trigger workflow when manually called - We keep this in case we want to manually re-sync packages with Glint for some reason
  workflow_dispatch:
  ...
  update-microservices:
    ...
    steps:
      ...
      - name: Run scanner script
        env:
          GLINT_CLIENT_ID: ${{ secrets.GLINT_CLIENT_ID }}
...
:::tip  
Click [**HERE**](https://github.com/kozmoai/demo-node-poject-npm) for the relevant files for this use-case.
:::
...
Glint makes it easy to implement and maintain package dependency mapping for you deployment configs. Using Glint's RESTful API and GitHub Workflows, you can easily integrate reporting to Glint with your existing CI/CD pipelines.
