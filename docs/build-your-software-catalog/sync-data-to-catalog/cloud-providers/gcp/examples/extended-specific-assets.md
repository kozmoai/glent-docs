...
resource "kozmoai_blueprint" "gcp_org_blueprint" {
  title      = "Organization"
  icon       = "GCP"
...
resource "kozmoai_entity" "gcp_org_entity" {
  identifier = data.google_organization.my_org.org_id
  title     = data.google_organization.my_org.domain
  blueprint = kozmoai_blueprint.gcp_org_blueprint.identifier
  properties = {
    string_props = {
...
resource "kozmoai_entity" "gcp_folder_entity" {
  ...
  blueprint  = kozmoai_blueprint.gcp_folder_blueprint.identifier
  properties = {
    string_props = {
...
resource "kozmoai_blueprint" "gcp_project_blueprint" {
  title      = "Project"
  icon       = "GCP"
  identifier = "project"
  properties = {
  ...
  relations = {
    organization = {
      target   = kozmoai_blueprint.gcp_org_blueprint.identifier
      title    = "Organization"
      many     = false
      required = false
    }
    folder = {
      target   = kozmoai_blueprint.gcp_folder_blueprint.identifier
      title    = "Folder"
      many     = false
      required = false
    }
...
resource "kozmoai_entity" "gcp_project_entity" {
  depends_on = [kozmoai_entity.gcp_org_entity, kozmoai_entity.gcp_folder_entity]
  for_each   = { for idx, project in data.google_projects.my_projects.projects : idx => project }
  identifier = each.value.project_id
  title      = each.value.name
  blueprint  = kozmoai_blueprint.gcp_project_blueprint.identifier
  properties = {
    string_props = {
...
resource "kozmoai_blueprint" "gcp_bucket_blueprint" {
  ...
  relations = {
    "project" = {
      target   = kozmoai_blueprint.gcp_project_blueprint.identifier
      title    = "GCP Project"
      required = false
      many     = false
    }
...
resource "kozmoai_entity" "gcp_bucket_entity" {
  depends_on = [kozmoai_entity.gcp_project_entity]
  for_each   = data.google_storage_bucket.my_buckets
  identifier = each.value.id
  title      = each.value.name
  blueprint  = kozmoai_blueprint.gcp_bucket_blueprint.identifier
  properties = {
    object_props = {
      labels = jsonencode(each.value.labels)
    }
...
resource "kozmoai_blueprint" "gcp_service_account_blueprint" {
  ...
  relations = {
    "project" = {
      title    = "Project"
      target   = "kozmoai_blueprint.gcp_project_blueprint"
      required = false
      many = false
    }
  }
...
resource "kozmoai_entity" "gcp_service_account_entity" {
  for_each   = data.google_service_account.my_accounts
  identifier = each.value.account_id
  title      = each.value.display_name
  blueprint  = kozmoai_blueprint.gcp_service_account_blueprint.identifier
  properties = {
    string_props = {
...
resource "kozmoai_blueprint" "gcp_disk_blueprint" {
  title      = "Disk"
...
resource "kozmoai_blueprint" "gcp_memorystore_blueprint" {
  title      = "Memorystore"
...
resource "kozmoai_entity" "gcp_memorystore_entity" {
  ...
  title      = each.value.name
  blueprint  = kozmoai_blueprint.gcp_memorystore_blueprint.identifier
  properties = {
    string_props = {
...
resource "kozmoai_blueprint" "gcp_compute_instance_blueprint" {
...
resource "kozmoai_entity" "gcp_compute_instance_entity" {
  for_each   = data.google_compute_instance.my_compute_instances
  identifier = "${each.value.project}_${each.value.zone}_${each.value.name}"
  title      = each.value.name
  blueprint  = kozmoai_blueprint.gcp_compute_instance_blueprint.identifier
  properties = {
    string_props = {
...
resource "kozmoai_blueprint" "gcp_run_service_blueprint" {
  title      = "Run Service"
  icon       = "Service"
...
resource "kozmoai_entity" "gcp_run_service_entity" {
  ...
  title      = each.value.name
  blueprint  = kozmoai_blueprint.gcp_run_service_blueprint.identifier
  properties = {
    string_props = {
...
resource "kozmoai_blueprint" "gcp_container_cluster_blueprint" {
...
resource "kozmoai_entity" "gcp_container_cluster_entity" {
  for_each   = data.google_container_cluster.my_container_clusters
  identifier = "${each.value.project}_${each.value.location}_${each.value.name}"
  title      = each.value.name
  blueprint  = kozmoai_blueprint.gcp_container_cluster_blueprint.identifier
  properties = {
    string_props = {
...
After running `terraform apply` you will see the `organization`, `folder`, `project`, `storageBucket`, `serviceAccount`, `disk`, `memorystore`, `computeInstance`, `runService`, `containerCluster` entities in Glint.
