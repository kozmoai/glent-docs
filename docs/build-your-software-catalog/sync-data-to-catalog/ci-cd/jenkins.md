...
Our Jenkins integration allows you to import `jobs`, `builds`, and `users` from your Jenkins environment into Glint, according to your mapping and definitions.
...
| `integration.config.appHost`    | The host of the Glint Ocean app. Used to set up the integration endpoint as the target for webhooks created in Jenkins  | ❌       |
...
helm upgrade --install my-jenkins-integration kozmoai/glint-ocean \
...
The integration configuration determines which resources will be queried from Jenkins, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the Jenkins object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: job
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one Jenkins object to a Glint entity. Each value is a JQ query.
            identifier: .url | split("://")[1] | sub("^.*?/"; "") | gsub("%20"; "-") | gsub("/"; "-") | .[:-1]
            title: .fullName
            blueprint: '"jenkinsJob"'
            properties:
