...
1. Create the following as variables in your Azure Devops project:
    1. Create the Glint credentials with the group id `glint-credentials`.
...
2. Create a Glint <GlintTooltip id="blueprint">blueprint</GlintTooltip> with the following JSON definition:
...
3. Create a Glint action in the [self-service page](https://app.useglint.io/self-serve) or with the following JSON definition:
...
4. Create the following Terraform templates in a `terraform` folder at the root of your GitHub repository:
    :::tip
    Fork our [example repository](https://github.com/kozmoai/pipelines-terraform-azure) to get started.
    :::
...
resources:
  webhooks:
    - webhook: GlintWebhook
      connection: GlintWebhook

variables:
  - group: glint-credentials
  - group: azure-service-principal
  - name: STORAGE_NAME
    value: ${{ parameters.GlintWebhook.payload.properties.storage_name }}
  - name: STORAGE_LOCATION
    value: ${{ parameters.GlintWebhook.payload.properties.storage_location }}
  - name: GLINT_RUN_ID
    value: ${{ parameters.GlintWebhook.context.runId }}

jobs:
- job: DeployJob
  displayName: 'Deploy to Azure and Glint'
  steps:
  - checkout: self
  ...
  - script: |
      ...
      tf_plan_and_apply glint
    displayName: 'Terraform changes to Azure and Glint'
    workingDirectory: 'terraform'
    env:
      TF_VAR_resource_group_name: arete-resources
      TF_VAR_port_client_id: $(GLINT_CLIENT_ID)
      TF_VAR_port_client_secret: $(GLINT_CLIENT_SECRET)
      TF_VAR_port_run_id: $(GLINT_RUN_ID)
...
6. Trigger the action from the [self-service](https://app.useglint.io/self-serve) page of your Glint application.
