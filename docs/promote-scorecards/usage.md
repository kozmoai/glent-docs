...
In order to create a scorecard from the API, you will make a PUT request to the URL `https://api.useglint.io/v1/blueprints/{blueprint_identifier}/scorecards`.
...
And then look at the [specific page](https://app.useglint.io/MicroserviceEntity?identifier=cart-service&activeTab=3) of this entity, on the scorecards tab
...
In order to create a scorecard from the [Terraform provider](../../build-your-software-catalog/sync-data-to-catalog/iac/terraform/) , you will need to use the `glint_scorecard` resource.

Here is an example of how to create an Ownership scorecard with the Terraform provider:

```hcl showLineNumbers
resource "glint_scorecard" "ownership" {
...
1. Update a single Scorecard using the URL `https://api.useglint.io/v1/blueprints/{blueprint_identifier}/scorecards/{scorecard_identifier}`. The request body will be the full Scorecard + the wanted changed values
2. Make a PUT request to the URL `https://api.useglint.io/v1/blueprints/{blueprint_identifier}/scorecards`. to multiple scorecards at once
...
In order to update a scorecard with the Terraform provider, you will need to run the `terraform apply -target=glint_scorecard.<resourceId>` command with the updated scorecard resource.
...
- Make an HTTP PUT request and remove it from the array of the scorecards via the URL `https://api.useglint.io/v1/blueprints/{blueprint_identifier}/scorecards`
- Make an HTTP DELETE request to the URL `https://api.useglint.io/v1/blueprints/{blueprint_identifier}/scorecards/{scorecard_identifier}` the `scorecard_identifier` is the identifier of the scorecard we want to delete

:::note
When using the multiple update Scorecards `https://api.useglint.io/v1/blueprints/{blueprint_identifier}/scorecards` PUT request, keep in mind that you will see a new `id` property. This is used via Glint to identify the Scorecard in order to be able to update its properties
:::

### From Terraform

In order to delete a scorecard using the Terraform provider, use the `terraform destroy -target=glint_scorecard.<resourceId>` command with the scorecard resource you want to delete. (remember that it is also possible to remove the definition of the `glint_scorecard` resource from the `.tf` file and run `terraform apply`)
