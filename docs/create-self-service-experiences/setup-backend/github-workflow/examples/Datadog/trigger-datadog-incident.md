...
This GitHub action allows you to quickly trigger incidents in Datadog via Glint Actions with ease.
...
2. Create a Datadog incident blueprint in Glint using the blueprint below:
...
jobs:
  create-entity-in-glint-and-update-run:
    ...
    steps:
      ...
      - name: Inform ingestion of Datadog incident into Glint
        uses: kozmoai/glint-github-action@v1
        with:
          clientId: ${{ secrets.GLINT_CLIENT_ID }}
          clientSecret: ${{ secrets.GLINT_CLIENT_SECRET }}
          ...
          operation: PATCH_RUN
          runId: ${{fromJson(inputs.port_payload).context.runId}}
          logMessage: Ingesting Datadog incident into Glint
      ...
      - name: Inform completion of Datadog incident ingestion into Glint
        uses: kozmoai/glint-github-action@v1
        with:
          clientId: ${{ secrets.GLINT_CLIENT_ID }}
          clientSecret: ${{ secrets.GLINT_CLIENT_SECRET }}
          ...
          logMessage: Finished request to ingest Datadog incident into Glint
...
Congrats 🎉 You've triggered your first incident in Datadog from Glint!