...
Glint provides granular control to ensure that every user only sees the parts of the catalog that are relevant for them.

Glint's catalog RBAC capabilities are enabled by utilizing [permissions controls](/sso-rbac/rbac/rbac.md).

:::tip Page permissions

In order to manage who can view certain **pages** in Glint, check out [page permissions](/customize-pages-dashboards-and-plugins/page/page-permissions.md).
:::
...
Refer to the [examples](./examples.md) page for practical examples of Glint's RBAC.

## FAQ

Since the catalog RBAC can be very granular, in some instances it might not be perfectly clear what the resulting assigned permissions would do, this part aims to provide some real-world examples and the behavior of Glint's RBAC in those instances.
