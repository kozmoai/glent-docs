...
Our Kubecost integration allows you to import `kubesystem` and `cloud` cost allocations from your Kubecost instance into Glint, according to your mapping and definition.
...
helm upgrade --install my-kubecost-integration kozmoai/glint-ocean \
...
The integration configuration determines which resources will be queried from Kubecost, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the Kubecost object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: kubesystem
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one Kubecost object to a Glint entity. Each value is a JQ query.
            identifier: .name
            title: .name
            blueprint: '"KubecostResourceAllocation"'
            properties:
...
<details>
<summary> Cost entity in Glint</summary>
