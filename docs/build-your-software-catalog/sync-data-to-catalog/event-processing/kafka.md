...
Our Kafka integration allows you to import `brokers` and `topics` from your Kafka `clusters` into Glint, according to your mapping and definition.

## Common use cases

- Map brokers and topics in your Kafka clusters.
- Watch for object changes (create/update/delete) on schedule, and automatically apply the changes to your entities in Glint.
...
helm upgrade --install kafka kozmoai/glint-ocean \
...
The integration configuration determines which resources will be queried from Kafka, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the Kafka object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: cluster
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one Kafka cluster to a Glint entity. Each value is a JQ query.
            identifier: .name
            title: .name
            blueprint: '"kafkaCluster"'
            properties:
