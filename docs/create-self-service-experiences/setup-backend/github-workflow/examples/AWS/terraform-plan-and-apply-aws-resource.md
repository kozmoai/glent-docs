...
3. Create two Glint self-service actions in the [self-service page](https://app.useglint.io/self-serve) on the `Service` blueprint with the following JSON definitions:

<details>

  <summary><b>Glint Action: Plan a Terraform Resource (click to expand)</b></summary>
   :::tip
...
<details>
  ...
  <summary><b>Glint Action: Approve and Apply Terraform Resource (click to expand)</b></summary>
   :::tip
- `<GITHUB-ORG>` - your GitHub organization or user name.
- `<GITHUB-REPO-NAME>` - your GitHub repository name.
...
jobs:
  plan-and-request-approval-for-bucket:
    ...
    steps:
      ...
      - name: Update Glint on successful plan and upload of terraform resource
      ...
      - name: Update Glint on unsuccessful plan of terraform resource
        if: ${{ steps.plan.outcome != 'success' || steps.artifact-upload-step.outcome != 'success' }}
        uses: kozmoai/glint-github-action@v1
        with:
          clientId: ${{ secrets.GLINT_CLIENT_ID }}
      ...
      - name: Log message to update Glint on the status of the TF Apply request (success)
        uses: kozmoai/glint-github-action@v1
      ...
      - name: Log message to update Glint on the status of the TF Apply request (failure)
        uses: kozmoai/glint-github-action@v1
        if: ${{ steps.request-tf-approval.outcome != 'success'}}
        with:
          clientId: ${{ secrets.GLINT_CLIENT_ID }}
    ...
    steps:
      ...
      - name: Log starting of cloud resource creation 
        ...
        with:
          ...
          runId: ${{ fromJson(inputs.port_payload).context.runId }}
          logMessage: |
              About to provision a cloud resource previously planned in Glint with run ID: ${{ github.event.inputs.port_run_identifier }} ... ⛴️
      ...
      - name: Update Glint on status of applying terraform resource (success)
        uses: kozmoai/glint-github-action@v1
      ...
      - name: Update Glint on status of applying terraform resource (failure)
        uses: kozmoai/glint-github-action@v1
        if: ${{steps.tf-apply.outcome != 'success'}}
        with:
          clientId: ${{ secrets.GLINT_CLIENT_ID }}
