...
By using Glint's generic webhook integration you can ingest data into the software catalog from any source or service that provides outgoing webhooks, even if Glint doesn't provide a native integration for that source.
...
Glint provides you with custom webhook endpoints, which you can use as the target for custom integrations provided by services you use (for example [GitHub](https://docs.github.com/en/webhooks-and-events/webhooks/about-webhooks), [Sentry](https://docs.sentry.io/product/integrations/integration-platform/webhooks/), etc.)
...
- The [security configuration](#security-configuration) used to make sure that payloads that arrive to Glint were really sent by a 3rd party you authorized.
...
The metadata configuration of the webhook includes all properties related to the visibility and displaying of the webhook inside Glint's UI, in addition to controlling whether the webhook is active or not.
...
The mapping configuration of the webhook defines how the webhook event payload is mapped to one (or more) Glint entities.

The mapping configuration makes use of the [JQ JSON processor](https://stedolan.github.io/jq/manual/) to extract information from the event payload into Glint entity properties.
...
- The `entity` key is used to map information from the webhook payload to Glint entity properties using JQ:
...
The security configuration of the webhook is used to tell Glint how to verify the hashed signature sent with the request from the 3rd party.
...
:::tip
The security configuration is not mandatory, but it does provide an additional layer of security, making sure that Glint only processes payloads that were actually sent from one of your 3rd party webhooks.
...
:::info
When using the `plain` algorithm, no hashing will be performed and the value of the secret saved in the Glint webhook configuration will be compared to the value in the specified header without any modification.
:::
...
- The `requestIdentifierPath` key is used to specify a JQ pattern resulting in a unique identifier of the webhook payload:
  - This key is used to prevent Glint from processing an event more than once;
...
To create a new webhook, make an HTTP POST request to `https://api.useglint.io/v1/webhooks` with your [webhook configuration](#configuration-structure) in the request body.
...
- `url` - this is the complete URL you need to pass to your 3rd party webhook configuration. Event payloads matching the webhook configuration you created should be sent to this URL;
  - The `url` will be of the format: `https://ingest.useglint.io/{webhookKey}`;
  - **Note:** The `https://ingest.useglint.io` is constant, only the `webhookKey` will change between webhooks configurations.

</TabItem>
<TabItem label="Update webhook" value="update">

To update an existing webhook, make an HTTP PATCH request to `https://api.useglint.io/v1/webhooks/{WEBHOOK_IDENTIFIER}` with your updated [webhook configuration](#configuration-structure) in the request body.
...
To delete an existing webhook, make an HTTP DELETE request to `https://api.useglint.io/v1/webhooks/{WEBHOOK_IDENTIFIER}`.

</TabItem>
</Tabs>
</TabItem>
<TabItem label="Using Glint UI" value="ui">
<Tabs>
<TabItem label="Create webhook" value="create-ui">

Follow these steps to create a webhook using Glint's UI:

1. Go to the [`Data sources`](https://app.useglint.io/dev-portal/data-sources) page of your portal.
...
7. Scroll down to box number 3. Here you can use <GlintTooltip id="jq">JQ</GlintTooltip> to map the data received from your system to the blueprints in your data model.
...
Follow these steps to update a webhook using Glint's UI:

1. Go the [`Data sources`](https://app.useglint.io/dev-portal/data-sources) page of your portal.
...
Follow these steps to delete a webhook using Glint's UI:
...
- Paste the webhook URL you received from Glint (`https://ingest.useglint.io/{webhookKey}`) in the field specifying the webhook target URL;
