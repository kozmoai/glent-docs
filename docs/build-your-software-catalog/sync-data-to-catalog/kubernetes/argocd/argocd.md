...
In this example you are going to create a webhook integration between [ArgoCD](https://argo-cd.readthedocs.io/en/stable/) and Glint, which will ingest application entities and map them to your ArgoCD projects.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map ArgoCD applications to Glint`;
   4. Icon : `Argo`;
...
To set up a webhook configuration in ArgoCD for sending notifications to Glint, follow these steps:
...
This command deploys the webhook notification configuration to your ArgoCD notification configmap (`argocd-notifications-cm`), allowing Glint to receive real-time events.

Done! any change that happens to your applications in ArgoCD will trigger a webhook event to the webhook URL provided by Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.

<h2>Argocd Events</h2>

In this example you are going to create a webhook integration between [ArgoCD](https://argo-cd.readthedocs.io/en/stable/) and Glint, which will ingest all events entities and map them to your ArgoCD applications.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map ArgoCD events to Glint`;
   4. Icon : `Argo`;
2. **Integration configuration** tab - fill the following JQ mapping:
