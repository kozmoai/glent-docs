...
description: Manage a developer environment and reflect it in Glint
---
...
In this example you are going to create a developer environment in AWS then report its information to Glint as a developer environment entity.
...
Since we are creating a lambda function, we need to provide its source code. You can download a basic Nodejs Lambda template by clicking [here](https://glint-docs-assets.s3.eu-west-1.amazonaws.com/lambda_function_payload.zip) (be sure to save the `.zip` file next to the `main.tf` file).
...
resource "aws_s3_bucket" "port_terraform_example_dev_env_bucket" {
  bucket = "my-glint-terraform-example-dev-env-bucket"
}
...
After running `terraform apply` you will see the resources in AWS, and the matching `developerEnvironment` entity in Glint.
...
To cleanup your environment, you can run the command `terraform destroy`, which will delete all of the resources you created in this example (including the AWS infrastructure and matching Glint entity).
