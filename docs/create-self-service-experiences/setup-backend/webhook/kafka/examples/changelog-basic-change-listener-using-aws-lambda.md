...
In this guide, we will show how to deploy a new `AWS Lambda function`, that will subscribe to the `change.log` topic and react to changes in our infrastructure as reported by Glint.
...
- Connection credentials to the Kafka topic provided to you by Glint, will look like this:
...
Whenever the amount of free storage gets too low, create a new execution function that frees up or extends the storage in your VM. Let’s learn how to do that, and what are Glint’s changelog capabilities.
...
def update_entity_prop_value(blueprint_identifier: str, identifier: str, property_name: str, property_value: Union[str, int]):
    '''
    Patches a Glint entity based on ``entity_props``
    '''
            ...
            try:
                ...
                if change_type == 'UPDATE' and resource_type == 'entity':
                    ...
                    if entity_total_storage * 0.1 > entity_free_storage:
                        ...
                        logger.info(f'Entity {entity_title} Storage freed up, updating in Glint')
...
aws lambda create-function --function-name glint-changelog-lambda \
...
```json showLineNumbers
{
    ...
    "FunctionArn": "arn:aws:lambda:us-east-2:123456789012:function:glint-changelog-lambda",
    "Runtime": "python3.9",
...
aws lambda update-function-configuration --function-name glint-changelog-lambda \
...
aws lambda update-function-configuration --function-name glint-changelog-lambda --environment "Variables={GLINT_CLIENT_ID=YOUR_CLIENT_ID,GLINT_CLIENT_SECRET=YOUR_CLIENT_SECRET}" --query "Environment"
```
...
```bash showLineNumbers
aws lambda update-function-configuration --function-name glint-changelog-lambda --environment file://environment.json --query "Environment"
```
...
aws lambda create-event-source-mapping --topics YOUR_ORG_ID.change.log --source-access-configuration Type=SASL_SCRAM_512_AUTH,URI=SECRET_ARN \
          --function-name glint-changelog-lambda \
...
Here is an example of the request payload received from Glint, inside the Kafka message (note the `before` and `after` keys showing the difference in our VM Entity properties):
...
If you want to dive even deeper into Glint's execution capabilities, you can explore other examples for Kafka-based Self-Service Actions.
