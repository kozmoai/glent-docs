...
In order to create a relation from the API, you will make a PUT request to the URL `https://api.useglint.io/v1/{source_blueprint_identifier}`.

The request flow is:

1. Construct the Relation object
2. Get the existing Blueprint schema from Glint API
...
In order to update a Relation from the API, you will make a PUT request to the URL `https://api.useglint.io/v1/{source_blueprint_identifier}`.
...
To rename the Relation's identifiers, you can make an HTTP PATCH request to the URL: `https://api.useglint.io/v1/{blueprint_identifier}/relations/{relation_identifier}/rename` with this following JSON:
...
- Make an HTTP PUT request to the URL `https://api.useglint.io/v1/{source_blueprint_identifier}` after removing the specific Relation object from `relations` key in the Blueprint schema.

## Next steps

Now that we understand **Relations**, we can begin to see how Glint helps us understand and manage our infrastructure layout.
