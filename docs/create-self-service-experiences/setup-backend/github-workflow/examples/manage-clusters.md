...
1. Prior knowledge of Glint Actions is essential for following this guide. Learn more about them [here](/create-self-service-experiences/setup-ui-for-action/).
...
5. A GitHub repository to contain your resources i.e. the github workflow file, glint resources, and infrastructure manifests. 


:::tip Starter Repository
Copy the `crossplane` folder from our examples repository [here](https://github.com/kozmoai/self-service-actions-examples) to follow along through the guide. The repository contains the following folders:
...
- `glint`: contains the Glint blueprints and action definitions.
...
To access the UI, Kubectl glint-forwarding can also be used to connect to the API server without exposing the service.

```bash
kubectl glint-forward svc/argocd-server -n argocd 8080:443
```
...
- Create a script named `create-cluster.sh` in the `scripts` folder of your repository to copy the template claim to the `infra` directory and replace placeholders with user inputs from Glint.
...
## 3. Glint Configuration

- Create a Glint blueprint that defines a data model for the cluster in the UI.
...
<br /> 
- On the [self-service](https://app.useglint.io/self-serve) page, create Glint actions to trigger GitHub actions for creating and deleting clusters.
...
spec:
  project: production
  source:
    repoURL: https://github.com/kozmoai-labs/crossplane-demo
    targetRevision: HEAD
    path: infra
  destination:
...
- On the [self-service](https://app.useglint.io/self-serve) page, go to the create cluster action and fill in the cluster properties.
...
Done! 🎉 You can now create and delete clusters from Glint.