...
* How to automate Slack reminders using Glint's self service actions
...
- You will need a Git repository in which you can place a workflow/pipeline that we will use in this guide. If you don't have one, we recommend creating a new repository named `Glint-actions`.
:::
...
In this guide we will create a self-service action that sends a Slack reminder for uncompleted [scorecard rules](https://docs.useglint.io/promote-scorecards/#what-is-a-scorecard). In reality, such an action can be used by R&D managers / Platform engineers to remind developers of unmet standards.
...
As part of the onboarding process, you should already have an action named `Send scorecard reminder` in your [self-service tab](https://app.useglint.io/self-serve). In that case, you can skip to the [Define action type](#define-backend-type) step.  
...
1. To get started, head to the [Self-service tab](https://app.useglint.io/self-serve) in your Glint application, and click on `New action`:

<img src='/img/guides/actionsCreateNew.png' width='50%' />

2. Each action in Glint is directly tied to a <GlintTooltip id="blueprint">blueprint</GlintTooltip>. Since we are sending a reminder for a service, we will use the `Service` blueprint we created in the [quickstart guide](/quickstart) from the dropdown.
...
Now we'll define the backend of the action:
  ...
  - Name the workflow `glint-slack-reminder.yml`.
...
3. Now let's create the workflow file that contains our logic. Under `.github/workflows`, create a new file named `glint-slack-reminder.yml` and use the following snippet as its content:

<details>
<summary><b>Github workflow (click to expand)</b></summary>

```yaml showLineNumbers
# glint-slack-reminder.yml
...
jobs:
    generate-scorecards-reminders:
        ...
        steps:
            - name: Generate Scorecards Reminders
              uses: kozmoai/glint-sender@v0.2.3
              with:
                operation_kind: scorecard_reminder
                port_client_id: ${{ secrets.GLINT_CLIENT_ID }}
                port_client_secret: ${{ secrets.GLINT_CLIENT_SECRET }}
                slack_webhook_url: ${{ secrets.SLACK_WEBHOOK_URL }}
                blueprint: service
                scorecard: ProductionReadiness
                target_kind: slack
            - name: Report status to Glint
...
:::tip Glint Initiatives sender Github action
This workflow uses Glint's [Initiatives Sender GitHub Action](https://github.com/marketplace/actions/glint-sender) to send a Slack message.
:::
...
3. Create a webhook in GitLab for triggering your GitLab:
  ...
  - Back in Glint, edit your action and in its `backend` step paste the webhook URL in the `Endpoint URL` field.
...
generate-scorecards-reminders:
  ...
  services:
    - docker:24.0.7-dind
  script:
    - image_name="ghcr.io/kozmoai/glint-sender:$VERSION"
    - echo "Generate Scorecards Reminders"
    - |
      docker run -i --rm --platform="linux/arm64/v8" \
      -e INPUT_GLINT_CLIENT_ID=$GLINT_CLIENT_ID \
      -e INPUT_GLINT_CLIENT_SECRET=$GLINT_CLIENT_SECRET \
...
💡 Note the `Log stream` at the bottom, this can be used to report progress, results and errors. Click [here](https://docs.useglint.io/create-self-service-experiences/reflect-action-progress/) to learn more.
:::
...
Congratulations! You can now send send Slack reminders easily from Glint 💪🏽
