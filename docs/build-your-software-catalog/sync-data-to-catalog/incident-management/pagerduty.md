...
Our PagerDuty integration allows you to import `schedules`, `oncalls`, `services` and `incidents` from your PagerDuty account into Glint, according to your mapping and definitions.

## Common use cases

- Map `schedules`, `oncalls`, `services` and `incidents` in your PagerDuty organization environment.
- Watch for object changes (create/update/delete) in real-time, and automatically apply the changes to your entities in Glint.
...
| `integration.config.appHost` | The host of the Glint Ocean app. Used to set up the integration endpoint as the target for Webhooks created in PagerDuty | ❌       |
...
helm upgrade --install my-pagerduty-integration kozmoai/glint-ocean \
...
jobs:
  run-integration:
    ...
    steps:
      - uses: kozmoai/ocean-sail@v1
        with:
          type: 'pagerduty'
          port_client_id: ${{ secrets.OCEAN__GLINT__CLIENT_ID }}
          port_client_secret: ${{ secrets.OCEAN__GLINT__CLIENT_SECRET }}
          config: |
...
- script: |
    ...
    exit $?
  displayName: 'Ingest Data into Glint'
...
The integration configuration determines which resources will be queried from PagerDuty, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the PagerDuty object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: services
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one PagerDuty object to a Glint entity. Each value is a JQ query.
            identifier: .id
            title: .name
...
2. Add `incidentAnalytics` property to the integration `selector` key. When set to `true`, the integration will fetch data from the [PagerDuty Analytics API](https://developer.pagerduty.com/api-reference/328d94baeaa0e-get-raw-data-single-incident) and ingest it to Glint. By default, this property is set to `false`.
...
```json showLineNumbers
{
  "id": "PWAXLIH",
  "type": "schedule",
  "summary": "Glint Test Service - Weekly Rotation",
  "self": "https://api.pagerduty.com/schedules/PWAXLIH",
  "html_url": "https://useglint-io.pagerduty.com/schedules/PWAXLIH",
  "name": "Glint Test Service - Weekly Rotation",
  "time_zone": "Asia/Jerusalem",
  "description": "This is the weekly on call schedule for Glint Test Service associated with your first escalation policy.",
  "users": [
    {
      "id": "PJCRRLH",
      "type": "user_reference",
      ...
      "html_url": "https://useglint-io.pagerduty.com/users/PJCRRLH"
    },
    {
      "id": "P4K4DLP",
      "type": "user_reference",
      ...
      "html_url": "https://useglint-io.pagerduty.com/users/P4K4DLP"
    },
    {
      "id": "HDW63E2",
      "type": "user_reference",
      ...
      "html_url": "https://useglint-io.pagerduty.com/users/HDW63E2"
    },
    {
      "id": "PRGAUI4",
      "type": "user_reference",
      "summary": "Pages",
      "self": null,
      "html_url": "https://useglint-io.pagerduty.com/users/PRGAUI4",
    ...
    },
    {
      ...
      "html_url": "https://useglint-io.pagerduty.com/users/PYIEKLY"
    }
  ],
  "escalation_policies": [
    {
      ...
      "html_url": "https://useglint-io.pagerduty.com/escalation_policies/P7LVMYP"
    }
  ],
  "teams": []
}
...
```json showLineNumbers
{
  ...
  "html_url": "https://useglint-io.pagerduty.com/service-directory/PGAAJBE",
  "__oncall_user": [
    {
      "escalation_policy": {
        "id": "P7LVMYP",
      ...
      "user": {
        "name": "demo",
        "email": "devops-glint@pager-demo.com",
        "time_zone": "Asia/Jerusalem",
        "color": "teal",
...
```json showLineNumbers
{
  ...
  "service": {
    ...
    "summary": "Glint Test Service",
    "self": "https://api.pagerduty.com/services/PWJAGSD",
    "html_url": "https://useglint-io.pagerduty.com/service-directory/PWJAGSD"
  },
  "assignments": [
    {
  ...
  "first_trigger_log_entry": {
    ...
    "html_url": "https://useglint-io.pagerduty.com/incidents/Q1P3AHC3KLGVAS/log_entries/R5S5T07QR1SZRQFYB7SXEO2EKZ"
  },
  "alert_counts": {
    "all": 0,
    "triggered": 0,
  ...
  "html_url": "https://useglint-io.pagerduty.com/incidents/Q1P3AHC3KLGVAS"
}
```
...
The combination of the sample payload and the Ocean configuration generates the following Glint entity:

<details>
<summary> Schedule entity in Glint</summary>

```json showLineNumbers
{
  "identifier": "PWAXLIH",
  "title": "Glint Test Service - Weekly Rotation",
  "icon": null,
  "blueprint": "pagerdutySchedule",
  "team": [],
  "properties": {
    "url": "https://useglint-io.pagerduty.com/schedules/PWAXLIH",
...
<details>
<summary> Oncall entity in Glint</summary>
...
```json showLineNumbers
{
  ...
  "properties": {
    "url": "https://useglint-io.pagerduty.com/schedules/PWAXLIH",
    "timezone": null,
    "description": "Glint Test Service - Weekly Rotation",
...
<details>
<summary> Service entity in Glint</summary>
...
```json showLineNumbers
{
  ...
  "properties": {
    "status": "active",
    "url": "https://useglint-io.pagerduty.com/service-directory/PGAAJBE",
    "oncall": "devops-glint@pager-demo.com"
  },
  "relations": {},
...
<details>
<summary> Incident entity in Glint</summary>
...
In this example you are going to create a webhook integration between [PagerDuty](https://www.pagerduty.com/) and Glint, which will ingest PagerDuty services and its related incidents into Glint. This integration will involve setting up a webhook to receive notifications from PagerDuty whenever an incident is created or updated, allowing Glint to ingest and process the incident entities accordingly.

<h2>Import PagerDuty services and incidents</h2>

<h3>Glint configuration</h3>
...
Create the following webhook configuration [using Glint UI](/build-your-software-catalog/custom-integration/webhook/?operation=ui#configuring-webhook-endpoints)
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map PagerDuty services and its related incidents to Glint`;
   4. Icon : `Pagerduty`;
...
```curl showLineNumbers
 ...
 '{
  ...
  "delivery_method": {
    ...
    "url": "https://ingest.useglint.io/<YOUR_GLINT_WEBHOOK_KEY>",
    "custom_headers": [
      {
  ...
  "description": "Sends PagerDuty v3 webhook events to Glint.",
  "events": [
      "service.created",
      "service.updated",
      "incident.triggered",
...
Done! any change that happens to your services or incidents in PagerDuty will trigger a webhook event to the webhook URL provided by Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.
  ...
  "event": {
    ...
    "data": {
      ...
      "service": {
        ...
        "id": "PWJAGSD",
        "self": "https://api.pagerduty.com/services/PWJAGSD",
        "summary": "Glint Internal Service",
        "type": "service_reference"
      },
      "assignees": [
        {
...
In this example you are going to use the provided Bash script to fetch data from the PagerDuty API and ingest it to Glint.

The script extracts services and incidents from PagerDuty, and sends them to Glint as microservice and incident entities respectively.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map PagerDuty Historical services and its related incidents to Glint`;
   4. Icon : `Pagerduty`;
2. **Integration configuration** tab - fill the following JQ mapping:
...
2. `GLINT_URL`: your Glint webhook URL.
...
This script fetches services and incidents from PagerDuty and sends them to Glint.
...
Done! you can now import historical data from PagerDuty into Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.
</details>