...
In the following example, you will create a changelog listener that can react to changes in Glint in a customized manner.
...
- Glint's changelog capabilities - to send an event to your API every time something changes in your software catalog;
...
For this limited example, the `deployment config` Blueprint will include mostly status properties that are likely to change frequently, thus showing Glint's changelog listener capabilities:
...
:::note
Remember to replace the placeholders for `YOUR_CLIENT_ID`, `YOUR_CLIENT_SECRET` and `YOUR_WEBHOOK_URL` with your Glint client ID, secret and your webhook URL (or Smee proxy URL).
:::
...
Now you're going to set up a full basic API to receive changelog events from your software catalog in Glint.
...
The code for the API is available in the [**changelog-listener-example-api**](https://github.com/kozmoai/glint-changelog-listener-example-api) repository, you can clone it and replace the `YOUR_CLIENT_ID`, `YOUR_CLIENT_SECRET` and `SLACK_WEBHOOK_URL` placeholders with your Glint client ID, secret and the Slack webhook URL you generated in [setting up a slack webhook](#setting-up-a-slack-webhook).
...
This example shows you the power of Glint's changelog capabilities.
