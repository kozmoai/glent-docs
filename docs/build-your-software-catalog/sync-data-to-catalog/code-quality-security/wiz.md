...
Our Wiz integration allows you to import `projects`, `issues`, `controls`, and `serviceTickets` from your Wiz account into Glint, according to your mapping and definitions.
...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------- |
| `glint.clientId`                 | Your glint client id ([Get the credentials](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials))                                                                                               | ✅       |
| `glint.clientSecret`             | Your glint client secret ([Get the credentials](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials))                                                                                           | ✅       |
...
| `integration.config.appHost`    | The host of the Glint Ocean app. Used to set up the integration endpoint as the target for Webhooks created in Wiz  | ❌       |
| `integration.secret.wizWebhookVerificationToken` | This is a password you create, that is used to verify webhook events to Glint                                       | ❌       |
...
helm upgrade --install my-wiz-integration kozmoai/glint-ocean \
...
The integration configuration determines which resources will be queried from Wiz, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the Wiz object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: project
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one Wiz object to a Glint entity. Each value is a JQ query.
            identifier: .id
...
In this example you are going to create a webhook integration between [Wiz](https://wiz.io/) and Glint, which will ingest Wiz issue entities into Glint.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map Wiz issues to Glint`;
   4. Icon : `Box`;
...
Done! Any issue created in Wiz will trigger a webhook event to the webhook URL provided by Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.
</details>
