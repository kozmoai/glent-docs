...
- The simplicity of communicating with Glint from a self-service action backend.
...
In this guide we will open a pull-request in our Git repository from within Glint to create a new cloud resource using gitops.
...
- Developers will be able to easily create and track cloud resources from Glint.

### Add a URL to your new resource's definition

In this guide we will add a new property to our `service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, which we can use to access our cloud resource definitions.

1. Go to your [Builder](https://app.useglint.io/dev-portal/data-model).
2. Click on your `service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, then click on `New property`.
...
1. Head to the [Self-service tab](https://app.useglint.io/self-serve) in your Glint application, and click on `+ New action`.

2. Each action in Glint is directly tied to a <GlintTooltip id="blueprint">blueprint</GlintTooltip>. Our action creates a resource that is associated with a service and will be provisioned as part of the service's CD process.  
   Choose `Service` from the dropdown list.

3. This action does not create/delete entities, but rather performs an operation on an existing <GlintTooltip id="entity">entity</GlintTooltip>. Therefore, we will choose `Day-2` as the action type.  
...
6. Now we'll define the backend of the action. Glint supports multiple invocation types, one of them should be selected for you depending on the Git provider you selected in the beginning of the onboarding process.
...
- Name the workflow `glint-create-bucket.yml`.
...
- Reporting & logging the action result back to Glint, and updating the relevant service's `Resource definitions` property with the URL of the service's resources directory.

Under `.github/workflows/`, create a new file named `glint-create-bucket.yml` and use the following snippet as its content:
...
fetch-glint-access-token: # Example - get the Glint API access token and RunId
  ...
  script:
    - |
      echo "Getting access token from Glint API"
      ...
      curl -X POST \
        ...
        "https://api.useglint.io/v1/actions/runs/$runId/logs"
      curl -X PATCH \
        -H 'Content-Type: application/json' \
        -H "Authorization: Bearer $accessToken" \
        -d '{"link":"'"$CI_PIPELINE_URL"'"}' \
        "https://api.useglint.io/v1/actions/runs/$runId"
  artifacts:
    reports:
      dotenv: data.env
...
create-entity:
  ...
  script:
    - |
      echo "Creating Glint entity to match new S3 bucket"
      ...
      curl -X POST \
          ...
          "https://api.useglint.io/v1/actions/runs/$RUN_ID/logs"
      curl --location --request POST "https://api.useglint.io/v1/blueprints/service/entities?upsert=true&run_id=$RUN_ID&create_missing_related_entities=true" \
...
update-run-status:
  ...
  script:
    - |
      echo "Updating Glint action run status and final logs"
      curl -X POST \
        -H 'Content-Type: application/json' \
        -H "Authorization: Bearer $ACCESS_TOKEN" \
...
pipeline {
    ...
    environment {
        ...
        GLINT_ACCESS_TOKEN = ""
        GLINT_BLUEPRINT_ID = "service"
        GLINT_RUN_ID = "${RUN_ID}"
        VISIBILITY="${VISIBILITY}"
        PR_URL=""
        SOURCE_REPO="glint-actions" // UPDATE WITH YOUR SOURCE REPO NAME
    }
...
- In your source repository (`glint-actions` for example), create a file named `cloudResource.tf` under `/templates/` (it's path should be `/templates/cloudResource.tf`).
...
After creating an action, it will appear under the `Self-service` tab of your Glint application:
...
#### Access the bucket's definition from Glint
...
2. Go to the <GlintTooltip id="entity">entity</GlintTooltip> page of the service that you executed the action for:
...
All done! You can now create resources for your services directly from Glint 💪🏽
...
With Glint, platform engineers can design precise and flexible self-service actions for their developers, while integrating with many different backends to suit your specific needs.

More relevant guides and examples:

- [Deploy AWS resources using AWS CloudFormation
  ](https://docs.useglint.io/create-self-service-experiences/setup-backend/github-workflow/examples/deploy-cloudformation-template)
- [Create an S3 bucket using Self-Service Actions](https://docs.useglint.io/create-self-service-experiences/setup-backend/webhook/examples/s3-using-webhook/)
