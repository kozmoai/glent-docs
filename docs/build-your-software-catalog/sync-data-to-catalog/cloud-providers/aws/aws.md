...
Our integration with AWS provides the ability to export your AWS resources to Glint, according to your configuration.
You can define the integration to run both on schedule and on events.

Our integration with AWS supports real-time event processing, this allows for an accurate **real-time** representation of your AWS infrastructure inside Glint.

:::tip
Glint's AWS exporter is open source, view the source code [**here**](https://github.com/kozmoai/glint-aws-exporter).
:::
...
- Use relations to create a complete, easily digestible map of your AWS accounts inside Glint.

## How it works

Glint's AWS exporter can retrieve all the resources supported by the [AWS Cloud Control API](https://docs.aws.amazon.com/cloudcontrolapi/latest/userguide/supported-resources.html).
...
The `config.json` file is how you specify the exact resources you want to export from your AWS account, and also which entities and which properties in Glint, you want to fill in with data.
...
- The `glint`, `entity` and the `mappings` keys open the section used to map the AWS resource fields to Glint entities, the `mappings` key is an array where each object matches the structure of an [entity](/build-your-software-catalog/sync-data-to-catalog/sync-data-to-catalog.md#entity-json-structure).
...
The S3 bucket is named `glint-aws-exporter-<AWS_REGION>-<AWS_ACCOUNT_ID>`, and is created by the exporter.
...
### Glint Credentials Secret

In order to manage entities in Glint, the exporter needs access to your Glint credentials, these are provided to the exporter via an [`AWS Secrets Manager`](https://docs.aws.amazon.com/secretsmanager/latest/userguide/intro.html) secret.
...
Either way, you need to add Glint's credentials to the secret on your own, when the secret is ready.
...
- [ASM secret](#glint-credentials-secret) - where you should save your Glint credentials (client id and secret), to allow the exporter to interact with Glint's API;
...
Continue to the [Installation](./Installation.md) section to setup the AWS exporter in your Glint environment.
