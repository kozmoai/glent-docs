...
Glint's
self-service actions. The Glint agent allows you to control the payload that is sent to every 3rd party application.

You can alter the requests sent to your third-party application by providing a payload mapping config file when
deploying the
Glint-agent container.
...
To provide the mapping configuration to the agent, run the [installation command](https://docs.useglint.io/create-self-service-experiences/setup-backend/webhook/glint-execution-agent/installation-methods/helm#installation) again, and add the following parameter:
...
To provide the mapping to the agent, add the mapping to the `values.yaml` file created in the installation [here](https://docs.useglint.io/create-self-service-experiences/setup-backend/webhook/glint-execution-agent/installation-methods/argocd#installation). The needs to be added as a top level field.
...
To provide the mapping to the agent, mount the mapping file to the container by adding the following parameter to the [installation command](https://docs.useglint.io/create-self-service-experiences/setup-backend/webhook/glint-execution-agent/installation-methods/docker#installation):
...
You can see examples showing how to deploy the Glint agent with different mapping configurations for various common use
cases below.

Each of the mapping fields can be constructed by JQ expressions. The JQ expression will be evaluated against the
original payload that is sent to the glint agent from Glint and the result will be sent to the 3rd party application.
...
[ # Can have multiple mappings. Will use the first one it will find with enabled = True (Allows you to apply mapping over multiple actions at once)
  {
      "enabled": bool || JQ,
      "url": JQ, # Optional. default is the incoming url from glint
      ...
      "body": ".body", # Optional. default is the whole payload incoming from Glint.
      "query": dict[str, JQ] # Optional. default is {},
      "report" { # Optional. Used to report the run status back to Glint right after the request is sent to the 3rd party application
...
Below you can find some mapping examples to demonstate how you can use JQ and the action payload sent from Glint to change the payload sent to your target endpoint by the agent.
...
## Report action status back to Glint

The agent is capable of reporting the action status back to Glint using the `report` field in the mapping.

The report request will be sent to the Glint API right after the request to the 3rd party application is sent and update
the run status in Glint.
...
- `externalRunId` - The external run id in the 3rd party application. The external run id is used to allow a search of
  the action runs in Glint by the external run id.
