...
Our integration with Kubernetes queries your Kubernetes clusters directly according to your definition. By using our Kubernetes integration, you can ingest live data directly from your K8s clusters into Glint in a transparent, efficient and precise manner, thus making sure only the information you need appears in the software catalog, and remains up to date.

Our integration with Kubernetes provides real-time event processing, this allows for an accurate **real-time** representation of your K8s cluster inside Glint.
...
:::tip
Glint's Kubernetes exporter is open source, view the source code [**here**](https://github.com/kozmoai/glint-k8s-exporter)
:::
...
- Use relations to create a complete, easily digestible map of your K8s cluster inside Glint;
- Map your Kubernetes resources from common CRDs such as ArgoCD, Istio and more;
- etc.

## How it works

Glint's Kubernetes exporter allows you to bring all the data supported by the K8s API to show running services, environments and more. The open source Kubernetes exporter allows you to perform extract, transform, load (ETL) on data from K8s into the desired software catalog data model.

The exporter is deployed using a Helm chart installed on the cluster. Once it is set up, it continues to sync changes, meaning that all changes, deletions or additions are accurately and automatically reflected in Glint.
...
resources: # List of K8s resources to list, watch, and export to Glint.
  - kind: apps/v1/replicasets # group/version/resource (G/V/R) format
    ...
    glint:
      entity:
        mappings: # Mappings between one K8s object to one or many Glint Entities. Each value is a JQ query.
...
- The `glint`, `entity` and the `mappings` keys open the section used to map the Kubernetes object fields to Glint entities, the `mappings` key is an array where each object matches the structure of an [entity](/build-your-software-catalog/sync-data-to-catalog/sync-data-to-catalog.md#entity-json-structure)
...
- Glint's Kubernetes exporter is installed using [Helm](https://helm.sh), so Helm must be installed to use the exporter's chart. Please refer to
  Helm's [documentation](https://helm.sh/docs) for installation instructions;
- You will need your [Glint credentials](/build-your-software-catalog/custom-integration/api/api.md#find-your-glint-credentials) to install the Kubernetes exporter.
...
:::info
The exporter helm chart can be found [here](https://github.com/kozmoai/helm-charts/tree/main/charts/glint-k8s-exporter)
:::
...
1. Add Glint's Helm repo by using the following command:
   ...
   :::tip
   If you already added Glint's Helm repo earlier, run `helm repo update` to retrieve the latest versions of the charts. You can then run `helm search repo kozmoai` to see the charts.
   :::

2. Install the exporter service on your Kubernetes cluster by running the following command:

   ```bash showLineNumbers
    helm upgrade --install my-glint-k8s-exporter kozmoai/glint-k8s-exporter \
        --create-namespace --namespace glint-k8s-exporter \
        --set secret.secrets.portClientId=YOUR_GLINT_CLIENT_ID --set secret.secrets.portClientSecret=YOUR_GLINT_CLIENT_SECRET \
        --set stateKey="k8s-exporter"  \
        --set eventListenerType="POLLING"  \
        --set "extraEnv[0].name"="CLUSTER_NAME" \
        --set "extraEnv[0].value"=YOUR_GLINT_CLUSTER_NAME
    ```
</TabItem>

<TabItem value="argo" label="ArgoCD">

1. Install the `my-glint-k8s-exporter` ArgoCD Application by creating the following `my-glint-k8s-exporter.yaml` manifest:
    :::note
    Remember to replace the placeholders for `LATEST_HELM_RELEASE` `YOUR_GLINT_CLIENT_ID` `YOUR_GLINT_CLIENT_SECRET` and `YOUR_GIT_REPO_URL`.
    
    You can find the latest version `glint-k8s-exporter` chart in our [Releases](https://github.com/kozmoai/helm-charts/releases?q=glint-k8s-exporter&expanded=true) page.
    ...
    kind: Application
    metadata:
      name: my-glint-k8s-exporter
      namespace: argocd
    spec:
      destination:
        namespace: my-glint-k8s-exporter
        server: https://kubernetes.default.svc
      project: default
      sources:
      - repoURL: 'https://kozmoai.github.io/helm-charts/'
        chart: glint-k8s-exporter
        // highlight-next-line
        targetRevision: LATEST_HELM_RELEASE
        helm:
          valueFiles:
            - $values/argocd/my-glint-k8s-exporter/values.yaml
          parameters:
...
```bash
kubectl apply -f my-glint-k8s-exporter.yaml
```
...
Done! The exporter will begin creating and updating objects from Kubernetes cluster as Glint entities shortly.

### Updating exporter configuration

To **update** the exporter resource mapping, open the [data sources](https://app.useglint.io/dev-portal/data-sources) page in Glint and click on your Kubernetes integration. Then edit the exporter configuration and click on the `Save & Resync` button.
