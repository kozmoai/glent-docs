...
This guide takes 10 minutes to complete, and aims to demonstrate Glint's flexibility when working with Gitops.

:::tip Prerequisites

- This guide assumes you have a Glint account and that you have finished the [onboarding process](/quickstart). We will use the `Service` blueprint that was created during the onboarding process.
- You will need a Git repository (Github, GitLab, or Bitbucket) in which you can place a workflow/pipeline that we will use in this guide. If you don't have one, we recommend creating a new repository named `Glint-actions`.
...
In this guide we will enrich a service in Glint using Gitops. In reality, this can be used by developers to independently add additional valuable data about their services to Glint.
...
Let's start by adding two new properties to the `Service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, that we will later populate using Gitops.

1. Go to your [Builder](https://app.useglint.io/dev-portal/data-model), expand the `Service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, and click on `New property`.
...
Services that share a business purpose (e.g. payments, shipping) are often grouped together using domains. Let's create a <GlintTooltip id="blueprint">blueprint</GlintTooltip> to represent a domain in Glint:

1. In your [Builder](https://app.useglint.io/dev-portal/data-model), click on the `Add` button in the top right corner, then choose `Custom blueprint`:
...
Now that we have a <GlintTooltip id="blueprint">blueprint</GlintTooltip> to represent a domain, let's connect it to our services. We will do this by adding a relation to the `Service` blueprint:

1. Go to your [Builder](https://app.useglint.io/dev-portal/data-model), expand the `Service` blueprint, and click on `New relation`:
...
Now that we have a `Domain` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, we can create some domains in Glint. This can be done manually from the UI, or via Gitops which is the method we will use in this guide.

1. In your `Glint-actions` (or equivalent) Github repository, create a new file named `glint.yml` in the root directory, and use the following snippet as its content:

<details>
<summary><b>glint.yml (click to expand)</b></summary>
...
2. Head back to your [software catalog](https://app.useglint.io/domains), you will see that Glint has created two new `domain` <GlintTooltip id="entity">entities</GlintTooltip>:
...
As part of the onboarding process, you should already have an action named `Enrich service` in your [self-service tab](https://app.useglint.io/self-serve). In that case, you can skip to the [Define backend type](#define-backend-type) step.  
...
1. Go to your [Self-service page](https://app.useglint.io/self-serve), then click on the `+ New action` button in the top right corner.

2. From the dropdown, choose the `Service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>.
...
Now we'll define the backend of the action. Glint supports multiple invocation types, for this tutorial we will use a `Github workflow`, a `GitLab pipeline`, or a `Jenkins pipeline`(choose this option if you are using Bitbucket).
...
<TabItem value="github">
  ...
  - You will need to have [Glint's Github app](https://github.com/apps/useglint-io) installed in your Github organization (the one that contains the repository you'll work with).
  - Replace the `Organization` and `Repository` values with your values (this is where the workflow will reside and run).
  - Name the workflow `glint-enrich-service.yml`.
...
<TabItem value="bitbucket">
    ...
    - You will need to have [Glint's Bitbucket app](https://marketplace.atlassian.com/apps/1229886/glint-connector-for-bitbucket?hosting=cloud&tab=overview) installed in your Bitbucket workspace (the one that contains the repository you'll work with).
...
Our action will create a pull-request in the service's repository, containing a `glint.yml` file that will add data to the service in Glint. Choose a backend type below to setup the workflow:
...
- Go to your [Glint application](https://app.useglint.io/), click on the `...` in the top right corner, then click `Credentials`. Copy your `Client ID` and `Client secret`.

2. In your `Glint-actions` (or equivalent) Github repository, create 3 new secrets under `Settings->Secrets and variables->Actions`:

- `ORG_ADMIN_TOKEN` - the personal access token you created in the previous step.
- `GLINT_CLIENT_ID` - the client ID you copied from your Glint app.
- `GLINT_CLIENT_SECRET` - the client secret you copied from your Glint app.
...
2. Create a new project named `Glint-pipelines`. Copy its GitLab project ID and replace the `(PROJECT_ID)` in the webhook URL . Then, under Settings->CI/CD, create a new Pipeline trigger token and use it to replace `(TRIGGER_TOKEN)` in the webhook URL.
...
- GLINT_CLIENT_SECRET  
  :::tip
  To find your Glint credentials, go to your [Glint application](https://app.useglint.io/), click on the `...` in the top right corner, then click `Credentials`.
  :::
...
2. Create the following [Jenkins credentials](https://www.jenkins.io/doc/book/using/using-credentials/#configuring-credentials
  ...
  <FindCredentials />
  - A `secret text` credential named `GLINT_CLIENT_ID` with your Glint client ID as the secret.
  - A `secret text` credential named `GLINT_CLIENT_SECRET` with your Glint client secret as the secret.

3. Create a Jenkins Pipeline with the following configuration:

  - [Enable webhook trigger](https://docs.useglint.io/create-self-service-experiences/setup-backend/jenkins-pipeline/#enabling-webhook-trigger-for-a-pipeline) for the pipeline.
  - [Define post-content variables](https://docs.useglint.io/create-self-service-experiences/setup-backend/jenkins-pipeline/#defining-variables) for the pipeline with the following names and values:
...
We will now create a YML file that will serve as a template for our services' `glint.yml` configuration file.
...
In the same repository, under `.github/workflows`, create a new file named `glint-enrich-service.yml` and use the following snippet as its content:
...
on:
  workflow_dispatch:
    inputs:
      ...
      port_payload:
        required: true
        description: Glint's payload, including details for who triggered the action and general context
        type: string
jobs:
  enrichService:
    ...
    steps:
      ...
      - name: Copy template yml file
        run: |
          cp templates/enrichService.yml ./targetRepo/glint.yml
      - name: Update new file data
        run: |
          sed -i 's/{{ service_identifier }}/${{fromJson(inputs.port_payload).context.entity}}/' ./targetRepo/glint.yml
          sed -i 's/{{ domain_identifier }}/${{ inputs.domain }}/' ./targetRepo/glint.yml
          sed -i 's/{{ service_type }}/${{ inputs.type }}/' ./targetRepo/glint.yml
          sed -i 's/{{ service_lifecycle }}/${{ inputs.lifecycle }}/' ./targetRepo/glint.yml
      - name: Open a pull request
        ...
        with:
          ...
          signoff: false
          branch: add-glint-yml
          delete-branch: true
          title: Create glint.yml - ${{fromJson(inputs.port_payload).context.entity}}
          body: |
            Add glint.yaml to enrich service in Glint.
          draft: false
      - name: Create a log message
        ...
        with:
          ...
          logMessage: Pull request to add glint.yml created successfully for service "${{fromJson(inputs.port_payload).context.entity}}" 🚀
```
...
enrichService:
  ...
  script:
    ...
    - >
      access_token=$(curl --location --request POST 'https://api.useglint.io/v1/auth/access_token' --header 'Content-Type: application/json' --data-raw "{\"clientId\": \"$GLINT_CLIENT_ID\",\"clientSecret\": \"$GLINT_CLIENT_SECRET\"}" | jq '.accessToken' | sed 's/"//g')
    - >
      curl --location --request PATCH "https://api.useglint.io/v1/actions/runs/$runID" --header "Authorization: $access_token" --header 'Content-Type: application/json' --data-raw "{\"link\": [\"$CI_PIPELINE_URL\"]
      }"
    ...
    - cp templates/enrichService.yml ./targetRepo/glint.yml
    - yq --in-place --arg SERVICE_ID $SERVICE_IDENTIFIER  '.[0].identifier = $SERVICE_ID' ./targetRepo/glint.yml -y
    - yq --in-place --arg TYPE $SERVICE_TYPE  '.[0].properties.type = $TYPE' ./targetRepo/glint.yml -y
    - yq --in-place --arg LIFECYCLE $SERVICE_LIFECYCLE '.[0].properties.lifecycle = $LIFECYCLE' ./targetRepo/glint.yml -Y
    - yq --in-place --arg DOMAIN $DOMAIN_IDENTIFIER  '.[0].relations.domain = $DOMAIN' ./targetRepo/glint.yml -Y
    - cd targetRepo
    - git pull
    - git checkout -b add-glint-yml
    - git add glint.yml
    - git commit -m "Enrich service - ${SERVICE_IDENTIFIER}"
    - set +e
    - output=$(git push origin add-glint-yml -o merge_request.create 2>&1)
    - echo "$output"
    - runStatus="SUCCESS"
    - |
    ...
    - |
      if [ "$runStatus" = "SUCCESS" ]; then
          ...
          curl --location --request POST "https://api.useglint.io/v1/actions/runs/$runID/logs" \
    ...
    - >
      curl --location --request PATCH "https://api.useglint.io/v1/actions/runs/$runID" --header "Authorization: $access_token" --header 'Content-Type: application/json' --data-raw "{\"status\": \"${runStatus}\"}"
```
...
Create a Jenkins pipeline script with the following content (replace `<GLINT-ACTIONS-REPO-URL>` with the URL of the repository you used in the previous step):
...
pipeline {
    ...
    stages {
        ...
        stage('checkoutTemplate') {
            steps {
                git credentialsId: 'BITBUCKET_CREDENTIALS', url: 'https://hadar-co@bitbucket.org/portsamples/glint-actions.git', branch: 'main'
            }
        }
        ...
        stage('copyTemplateFile') {
            steps {
                sh 'cp templates/enrichService.yml ./destinationRepo/glint.yml'
            }
        }
        stage('updateFileData') {
            steps {
                sh """#!/bin/bash
                    sed -i .bak 's/{{ service_identifier }}/${ENTITY_IDENTIFIER}/' ./destinationRepo/glint.yml
                    sed -i .bak 's/{{ domain_identifier }}/${DOMAIN}/' ./destinationRepo/glint.yml
                    sed -i .bak 's/{{ service_type }}/${TYPE}/' ./destinationRepo/glint.yml
                    sed -i .bak 's/{{ service_lifecycle }}/${LIFECYCLE}/' ./destinationRepo/glint.yml
                """
            }
        }
        stage('CreateBranch') {
                ...
                dir('destinationRepo') {
                   script {
                        sh '''#!/bin/bash
                            ...
                            -F message="Add glint.yml" -F branch=add-glint-yml \
                            -F glint.yml=@glint.yml
                        '''
                    }
                }
            }
...
1. After creating an action, it will appear under the [Self-service page](https://app.useglint.io/self-serve). Find your new `Enrich service` action, and click on `Execute`.
...
5. Merge the pull-request, then head back to your [software catalog](https://app.useglint.io/services).
...
Glint allows you to easily integrate your Gitops practices with your software catalog, reflecting the state of your infrastructure, and allowing you to empower your developers with controlled actions.
