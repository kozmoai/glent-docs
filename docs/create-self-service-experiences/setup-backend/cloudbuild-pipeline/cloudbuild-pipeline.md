...
In this guide, you will learn how to trigger your [Cloud Build](https://cloud.google.com/build) Pipelines from Glint, using [Webhook Actions](../webhook/).
...
3. Glint publishes an invoked `WEBHOOK` via a `POST` request to `https://cloudbuild.googleapis.com/v1/projects/{project_id}/triggers/{webhook_name}:webhook?key={google_api_key}&secret={webhook_secret}`

An example flow would be:

1. A developer asks to run a Cloud Build pipeline;
2. Glint sends a `POST` request with the action payload to the Cloud Build webhook `URL`;
...
**Glint Action** - The full Glint action definition can be found [here](#setting-up-the-glint-action).
:::

### Setting up the Glint action

To trigger the Cloud Build pipeline, you will setup a Glint [Webhook Action](../webhook/).
...
### Report CloudBuild run status to Glint

Once you have triggered your Cloud Build pipeline successfully, it is essential to update the status of the run action in Glint. This update allows Glint to monitor the status of your Cloud Build pipeline.

The code snippet below demonstrates how you can report the progress of your pipeline to Glint. Remember to modify the Glint credentials in the substitutions section for Cloud Build to authenticate using your Glint access token.

```yaml showLineNumbers
steps:
  # Get Glint's Access Token
    ...
    args:
      ...
      - |
        ...
        accessToken=$(curl -X POST \
          ...
          -d '{"clientId": "${_GLINT_CLIENT_ID}", "clientSecret": "${_GLINT_CLIENT_SECRET}"}' \
          -s 'https://api.useglint.io/v1/auth/access_token' | grep -o '"accessToken":"[^"]*' | awk -F'"' '{print $4}')
    ...
    args:
      ...
      - |
        curl -X POST \
          ...
          'https://api.useglint.io/v1/actions/runs/${_RUN_ID}/logs'
    entrypoint: bash

  # Glint status update example
  - name: "gcr.io/cloud-builders/curl"
    args:
      - "-c"
      - |
        curl -X PATCH \
          ...
          'https://api.useglint.io/v1/actions/runs/${_RUN_ID}'
    entrypoint: bash

substitutions:
  _RUN_ID: $(body.context.runId)
  _GLINT_CLIENT_ID: <Your Glint Client Id>
  _GLINT_CLIENT_SECRET: <Your Glint Client Secret>
...
That's it! Anytime a user invokes an action in Glint UI, a webhook trigger will be sent to Google Cloud Build to execute the pipeline.

For more information, visit:

- [Cloud Build example repository](https://github.com/kozmoai/glint-cloudbuild-webhook-trigger-example)
