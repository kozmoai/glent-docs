...
Our integration with [Terraform](https://www.terraform.io/) allows you to combine the state of your infrastructure with the entities representing them in Glint.

By using Glint's Terraform provider you make it easy to integrate Glint with your existing IaC definitions, every resource provisioned by Terraform can also be reported to the software catalog using the same `.tf` definition file.

:::info glint terraform provider
You can view the official registry page for our Terraform provider [here](https://registry.terraform.io/providers/kozmoai/kozmoai/)
:::
...
Glint's Terraform provider supports The following resources to ingest data to the catalog:

### `port_entity`

The [`port_entity`](https://registry.terraform.io/providers/kozmoai/kozmoai/latest/docs/resources/port_entity) resource defines a basic entity:
...
The [`properties`](https://registry.terraform.io/providers/kozmoai/kozmoai/latest/docs/resources/port_entity#nested-schema-for-properties) schema assigns a specified value to one of the entity's properties.
...
The [`relations`](https://registry.terraform.io/providers/kozmoai/kozmoai/latest/docs/resources/port_entity#relations) schema maps a target entity to the source entity definition:
...
To ingest data to the software catalog using the Terraform provider, you will define [`port_entity`](https://registry.terraform.io/providers/kozmoai/kozmoai/latest/docs/resources/port_entity) resources in your Terraform definition files:
...
It is also possible to start managing existing entities using Glint's Terraform provider, to begin managing an existing entity, add a new `port_entity` resource to your `.tf` definition file and make the desired changes:
...
- Glint's Terraform provider uses the [create/override](/build-your-software-catalog/custom-integration/api?update-strategy=create-override#usage) strategy, meaning for an existing entity, any properties not defined in the resource definition will be overridden with empty values.
...
Before using `terraform import` to bring data from your Glint account into your Terraform state file, make sure your resource definitions match the schema of your resources in Glint. If they don't, your state will be deleted in the next `terraform apply`, since Terraform will try to apply the empty resources and override the imported state, while also updating Glint in the process.                                                                                                                                          
:::
