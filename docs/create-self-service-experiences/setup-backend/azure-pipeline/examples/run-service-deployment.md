...
In the following guide, you are going to build a self-service action in Glint that executes an Azure pipeline behind the scenes.
...
To learn how to generate an API token to interact with Glint, look [here](../../../../build-your-software-catalog/custom-integration/api/#generate-an-api-token).

To update the status of the action in Glint, use the following API call in your Azure pipeline:
...
curl -X PATCH \
 ...
 -d '{"status": "SUCCESS"}' \
 https://api.useglint.io/v1/actions/runs/YOUR_RUN_ID
```
