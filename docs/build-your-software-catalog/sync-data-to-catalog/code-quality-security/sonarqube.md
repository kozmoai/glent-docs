...
Our SonarQube integration (powered by [Ocean](https://ocean.useglint.io)) allows you to import `projects`, `issues` and `analyses` from your SonarQube account into
Glint, according to your mapping and definitions.
...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | -------- |
| `glint.clientId`                          | Your glint client id ([How to get the credentials](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials))                                 |                                     | ✅       |
| `glint.clientSecret`                      | Your glint client secret ([How to get the credentials](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials))                             |                                     | ✅       |
...
helm upgrade --install my-sonarqube-integration kozmoai/glint-ocean \
...
- script: |
    ...
    exit $?
  displayName: 'Ingest SonarQube Data into Glint'
...
<details>
<summary> Project entity in Glint</summary>
...
<details>
<summary> Issue entity in Glint</summary>
...
<details>
<summary> Analysis entity in Glint</summary>
...
In this example you are going to create a webhook integration between [SonarQube's SonarCloud](https://www.sonarsource.com/products/sonarcloud/) and Glint, which will ingest SonarQube code quality `analysis` entities.

<h2> Glint configuration </h2>
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map SonarQube alerts to Glint`;
   4. Icon : `sonarqube`;
...
Done! any new analysis you run (for example, on new PRs or changes to PRs) will trigger a webhook event that SonarCloud will send to the webhook URL provided by Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.
...
This guide aims to demonstrate how to connect a SonarQube project to an existing service in Glint.
...
5. To add a new tag, click on the **plus** icon and type the tag name (`glint-auth-service`) into the input field provided. For this guide, let's assume there is a service entity identified by `auth-service` in your `Service` blueprint in Glint. 

<img src='/img/guides/sonarProjectAddTags.png' width='60%' />

:::note Control the tag name
Since our `SonarQube project` may already have several tags, we will need a mechanism to control how these tags will be related to our `Service` blueprint. A way to achieve this relation is to prefix the tag name with the keyword `glint-`. We will then use JQ to select the tags that starts with this keyword. So, our example tag will be named `glint-auth-service`, which will correspond to a Service entity identified by `auth-service` in Glint.
:::


### Create the service relation

Now that Glint is synced with our SonarQube resources, let's reflect the SonarQube project in our services to display the projects used in a service.
...
1. Head back to the [Builder](https://app.useglint.io/dev-portal/data-model), choose the `SonarQube Project` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, and click on `New relation`:
...
Now that the <GlintTooltip id="blueprint">blueprints</GlintTooltip> are related, we need to assign the relevant SonarQube project to each of our services. This can be done by adding some mapping logic. Go to your [data sources page](https://app.useglint.io/dev-portal/data-sources), and click on your SonarQube integration:
      ...
      entity:
        mappings:
          ...
          properties:
            ...
            mainBranch: .__branch.name
          relations:
            service: .tags | map(select(startswith("glint"))) | map(sub("glint-"; ""; "g")) | .[0]
```
...
The JQ below selects all tags that start with the keyword `glint`. It then removes "glint-" from each tag, leaving only the part that comes after it. It then selects the first match, which is equivalent to the service in Glint.
...
By following these steps, you can seamlessly connect a SonarQube project with an existing service blueprint in Glint using project tags.
