...
In this example you are going to create a webhook integration between [Dynatrace](https://www.dynatrace.com/) and Glint, which will ingest problem entities to Glint and map them to your microservice entities.
...
:::note
The webhook configuration's relation mapping will function properly only when the identifiers of the Glint microservice entities match the names of the entities in your Dynatrace.

If there is a mismatch, you can utilize [Dynatrace Tags](https://www.dynatrace.com/support/help/manage/tags-and-metadata) to align the actual identifier in Glint.

To do this, create a tag with the key proj and value `microservice_identifier`.

Then, update the relation JQ syntax to establish a connection between the Dynatrace problem and the Glint microservice. Here is the updated JQ Mappings:
...
6. Input the following details:
   1. `Display name` - use a meaningful name such as Glint Webhook;
...
Done! any problem detected on your Dynatrace entity will trigger a webhook event. Glint will parse the events according to the mapping and update the catalog entities accordingly.
...
- [Code Repository Example](https://github.com/kozmoai/example-dynatrace-entities)
