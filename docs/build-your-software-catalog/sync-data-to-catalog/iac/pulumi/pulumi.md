...
Our integration with [Pulumi](https://www.pulumi.com/) allows you to combine the state of your infrastructure with the entities representing them in Glint.

By using Glint's Pulumi provider you make it easy to integrate Glint with your existing IaC definitions, every resource provisioned by Pulumi can also be reported to the software catalog using the same definition file.

<!-- :::info glint pulumi provider
...
Glint's Pulumi provider supports The following resources to ingest data to the catalog:
...
import { Entity } from "@kozmoai/glint";
...
func main() {
    pulumi.Run(func(ctx *pulumi.Context) error {
        entity, err := port_pulumi.NewEntity(ctx, "myEntity", &port_pulumi.EntityArgs{
            ...
            Properties: glint.EntityPropertiesArgs{},
            Relations:  glint.EntityRelations{},
        })
        if err != nil {
            return err
        }
...
const entity = new glint.Entity("myEntity", {
  identifier: "myEntity",
  title: "My Entity",
  blueprint: "myBlueprint",
  // highlight-start
...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		entity, err := glint.NewEntity(ctx, "entity", &glint.EntityArgs{
...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		entity, err := glint.NewEntity(ctx, "entity", &glint.EntityArgs{
			...
			// highlight-start
            Properties: glint.EntityPropertiesArgs{
              ArrayProps: glint.EntityPropertiesArrayPropsArgs{
...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		entity, err := glint.NewEntity(ctx, "entity", &glint.EntityArgs{
			...
			Blueprint:  pulumi.String("myBlueprint"),
			// highlight-start
            Relations: glint.EntityRelationsArgs{
...
:::note
At the moment, it is only possible to create entities with `many: false` relations using Glint's Pulumi provider.
:::

## Ingest data using the Pulumi provider

To ingest data to the software catalog using the Pulumi provider, you will create an instance of `glint.Entity` resource in your preferred langauge:
...
To update an entity using Pulumi, update the existing `glint.Entity` resource in code file and then run `pulumi up -y`.

It is also possible to start managing existing entities using Glint's Pulumi provider, to begin managing an existing entity, add a new `glint.Entity` resource to your code file and make the desired changes
...
- Glint's Pulumi provider uses the [create/override](/build-your-software-catalog/custom-integration/api?update-strategy=create-override#usage) strategy, meaning for an existing entity, any properties not defined in the resource definition will be overridden with empty values.
...
To delete an entity using Pulumi, simply remove the `glint.Entity` resource defined in code file and then run `pulumi up -y`.
