...
```json showLineNumbers
{
  "ReportName": "aws-monthly-cost-report-for-glint",
  "TimeUnit": "MONTHLY",
  "Format": "textORcsv",
  "Compression": "GZIP",
...
jobs:
  run:
    ...
    steps:
      - name: run
        uses: docker://ghcr.io/kozmoai/glint-aws-cost-exporter:latest
        env:
...
run_job:
  stage: run
  script:
    - docker run -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e AWS_BUCKET_NAME=$AWS_BUCKET_NAME -e GLINT_CLIENT_ID=$GLINT_CLIENT_ID -e GLINT_CLIENT_SECRET=$GLINT_CLIENT_SECRET ghcr.io/kozmoai/glint-aws-cost-exporter:latest
  rules:
