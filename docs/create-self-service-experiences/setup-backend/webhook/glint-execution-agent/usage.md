...
:::note
**IMPORTANT**: To make use of the **Glint execution agent**, you need to configure:
...
Well Done! **Glint Agent** is now running in your environment and will trigger any webhook that you've configured (for self-service actions, or changes in the software catalog).
...
![Glint Execution Agent Logs](/img/self-service-actions/glint-execution-agent/glintAgentLogs.png)


## Advanced configuration
Some environments require special configuration when working with the Glint agent. This includes working with self-signed certificates and/or proxies.

Glint's agent uses Python's [requests](https://requests.readthedocs.io/en/latest/) library. This allows passing advanced configuration using environment variables.

To add an environment variable using the agent's Helm chart, either:

1. Using Helm's `--set` flag:
```sh showLineNumbers
helm upgrade --install <MY_INSTALLATION_NAME> kozmoai/glint-ocean \
