...
This page will help you install Glint's GitLab integration (powered by the Ocean framework).
...
In order for the GitLab integration to update the data in Glint on every change in the GitLab repository, you need to specify the `appHost` parameter.
...
The GitLab integration supports listening to GitLab webhooks and updating the relevant entities in Glint accordingly.
...
- The system hooks has much less event types than the group webhooks.
  ...
  - System Hooks supported event types:
    ...
    This means that if you choose system hooks, the integration will not be able to update the relevant entities in Glint on events such as `issues` or `pipeline`.
...
| `integration.config.appHost`       | The host of the Glint Ocean app. Used to set up the integration endpoint as the target for webhooks created in GitLab                | https://my-ocean-integration.com | ❌       |
...
helm upgrade --install my-gitlab-integration kozmoai/glint-ocean \
...
It is also possible to get Glint's UI to generate your installation command for you, Glint will inject values such as your Glint client ID and client secret directly into the command, making it easier to get started.

Follow these steps to setup the integration through Glint's UI:

1. Click the ingest button in Glint Builder Page for the blueprint you want to ingest using GitLab:
...
variables:
  ...
  # OCEAN__GLINT__CLIENT_ID: $OCEAN__GLINT__CLIENT_ID
  # OCEAN__GLINT__CLIENT_SECRET: $OCEAN__GLINT__CLIENT_SECRET
