...
- Some advanced types of properties that can be added to <GlintTooltip id="blueprint">blueprints</GlintTooltip>, and what can be achieved by using them.
- The value and flexibility of scorecards in Glint.
...
In this guide we will add two new properties to our `service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, which we will then use to set production readiness standards:
...
In this guide we will use Pagerduty to get our services' on-call. Note that Glint also has integrations for other incident response platforms.
...
#### Integrate Pagerduty into Glint

Now let's bring our Pagerduty data into Glint. Glint's Pagerduty integration automatically fetches `Services` and `Incidents`, and creates <GlintTooltip id="blueprint">blueprints</GlintTooltip> and <GlintTooltip id="entity">entities</GlintTooltip> for them.
...
1. Install Glint's Pagerduty integration using Helm, by running the command below in your terminal.

:::tip Alternative installation
The command below will install the integration in `Realtime & always on` mode. If you prefer to use a one-time (scheduled) installation instead, see the [Pagerduty installation](https://docs.useglint.io/build-your-software-catalog/sync-data-to-catalog/incident-management/pagerduty/?installation-methods=one-time#installation) section.
:::

- Replace `CLIENT_ID` and `CLIENT_SECRET` with your credentials (get them [here](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials)).
...
helm upgrade --install my-pagerduty-integration kozmoai/glint-ocean \
    --set glint.clientId="CLIENT_ID" \   # REPLACE VALUE
    --set glint.clientSecret="CLIENT_SECRET"  \   # REPLACE VALUE
...
Great! Now that the integration is installed, we should see some new components in Glint:

- Go to your [Builder](https://app.useglint.io/dev-portal/data-model), you should now see two new <GlintTooltip id="blueprint">blueprints</GlintTooltip> created by the integration - `PagerDuty Service` and `PagerDuty Incident`.
- Go to your [Software catalog](https://app.useglint.io/services), click on `PagerDuty Services` in the sidebar, you should now see a new <GlintTooltip id="entity">entity</GlintTooltip> created for our `DemoPdService`, with a populated `On-call` property.

#### Add an on-call property to the service blueprint

Now that Glint is synced with our Pagerduty resources, let's reflect the Pagerduty service's on-call in our services.  
...
1. Head back to the [Builder](https://app.useglint.io/dev-portal/data-model), choose the `Service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, and click on `New relation`:
...
Now that the <GlintTooltip id="blueprint">blueprints</GlintTooltip> are related, let's create a [mirror property](https://docs.useglint.io/build-your-software-catalog/customize-integrations/configure-data-model/setup-blueprint/properties/mirror-property/) in our service to display its on-call.

1. Choose the `Service` <GlintTooltip id="blueprint">blueprint</GlintTooltip> again, and under the `PagerDuty Service` relation, click on `New mirror property`.  
...
2. Now that our mirror property is set, we need to assign the relevant Pagerduty service to each of our services. This can be done by adding some mapping logic. Go to your [data sources page](https://app.useglint.io/dev-portal/data-sources), and click on your Pagerduty integration:
...
1. Go to your [Software catalog](https://app.useglint.io/services), choose any service in the table under `Services`, click on the `...`, and click `Edit`:
...
1. Go to your [Builder](https://app.useglint.io/dev-portal/data-model) again, choose the `Service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, and click `New property`.
...
3. Next we will update the Github exporter mapping and add the new property. Go to your [data sources page](https://app.useglint.io/dev-portal/data-sources).
...
_Remember the `identifier` from step 2? This tells Glint how to populate the new property_ 😎

Going back to our Catalog, we can now see that our <GlintTooltip id="entity">entities</GlintTooltip> have their code owners displayed:
...
1. Go to your [Builder](https://app.useglint.io/dev-portal/data-model), choose the `Service` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, click on `Scorecards`, then click our existing `Production readiness` scorecard:
...
- Use Glint's API to check for scorecard compliance from your CI and pass/fail it accordingly.
...
With Glint, standards are easy to set-up, prioritize and track. Using Glint's API, you can also create/get/modify your scorecards from anywhere, allowing seamless integration with other platforms and services in your environment.

More relevant guides and examples:

- [Glint's OpsGenie integration](https://docs.useglint.io/build-your-software-catalog/sync-data-to-catalog/incident-management/opsgenie/)
