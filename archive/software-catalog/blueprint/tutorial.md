...
Let's see how we can use Glint's API to create Blueprints:
...
To find your Glint API credentials go to [Glint](https://app.useglint.io), hover on the `3 dots button` at the top right corner, and select `Credentials` then you will be able to view and copy your `CLIENT_ID` and `CLIENT_SECRET`:
...
In order to perform any action with Glint's API, you first need an **access token**. Below are some code examples from various languages:
...
In order to interact with the Blueprints API our basic URL will be [https://api.useglint.io/v1/blueprints](https://api.useglint.io/v1/blueprints), and we will create the Blueprint using an HTTP **POST** request.

:::note
We will use the [access token](#getting-an-api-token) we generated earlier when making new requests to Glint's API.
...
curl --location --request POST "https://api.useglint.io/v1/blueprints" \
...
- Make an HTTP PUT request to the URL: `https://api.useglint.io/v1/{blueprint_identifier}`
...
Make an HTTP PUT request to the URL `https://api.useglint.io/v1/blueprints/{old_blueprint_identifier}` with the following JSON body:
...
Make an HTTP PATCH request to the URL `https://api.useglint.io/v1/blueprints/{old_blueprint_identifier}` with the following JSON body:
...
In order to rename the properties, you can make an HTTP PATCH request to the URL: `https://api.useglint.io/v1/{blueprint_identifier}/properties/{property_name}/rename` with this following JSON:
...
- Make an HTTP DELETE request to the URL `https://api.useglint.io/v1/{blueprint_identifier}`
