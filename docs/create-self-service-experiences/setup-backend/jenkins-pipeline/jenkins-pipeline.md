...
In this guide, you will learn how to trigger your [Jenkins](https://www.jenkins.io/) Pipelines from Glint, using [Webhook Actions](../webhook/).

![Illustration](../../../../static/img/self-service-actions/setup-backend/jenkins-pipeline/jenkins-illustration.png)

The steps shown in the image above are as follows:

1. An action is invoked in Glint;
2. Glint signs the action payload using SHA-1 with the [`clientSecret`](../../../build-your-software-catalog/custom-integration/api/api.md#find-your-glint-credentials) value and puts it in the `X-Glint-Signature` request header.
...
3. Glint publishes an invoked `WEBHOOK` via a `POST` request to `https://{JENKINS_URL}/generic-webhook-trigger/invoke`

   An example flow would be:

   1. A developer asks to run a Jenkins pipeline;
   2. Glint sends a `POST` request with the action payload to the Jenkins webhook `URL`;
...
- The `Expression` field should be set to `JSONPath` and be directed to the relevant property sent by the Glint action.
...
:::tip
Here is part of the JSON scheme of the Glint action, which shows the inputs sent by Glint when triggering the action:
...
**Glint Action** - The full Glint action definition can be found [here](./jenkins-pipeline.md#setting-up-the-glint-action).
...
### Setting up the Glint action

To trigger the Jenkins pipeline, you will setup a Glint [Webhook Action](../webhook/).
...
- Add [validation](../webhook/signature-verification.md) for the webhook payload content to verify that it really originated from Glint.
...
- The IP field should be set to `3.251.12.205`, which is our hosted outbound WEBHOOK Gateway;
  - For more information about Glint's outbound calls, check out Glint's [actions security](../../security/security.md) page.
- In the **HMAC Secret** field, choose a secret containing your `glint-client-secret`.

If this secret doesn't already exist, create a `secret text` type secret using [this guide](https://www.jenkins.io/doc/book/using/using-credentials/). The value of the secret should be your `Glint Client Secret` which can be found by following the guide [here](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials).
:::

### Report Jenkins action run status to Glint

Once you have triggered your Jenkins pipeline successfully, it is essential to update the status of the run action in Glint.
...
The code snippet below demonstrates how you can report the progress of your pipeline to Glint:
...
pipeline {
    ...
    environment {
        GLINT_CLIENT_ID="YOUR_CLIENT_ID"
        GLINT_CLIENT_SECRET="YOUR_CLIENT_SECRET"
        ACCESS_TOKEN = ""
    }
