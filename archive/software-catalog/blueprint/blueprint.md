...
A Blueprint is _our basic_ building block in Glint. It represents assets that can be managed in Glint, such as `Microservice`, `Environments`, `Packages`, `Clusters`, `Databases`, and many more.
...
| ----------- | --------------------------------------------------------- | ----------------------------------------- |
| `url`       | Formatted URL                                             | `"https://useglint.io"`                    |
| `email`     | Formatted Email                                           | `"glint@useglint.io"`                       |
| `user`      | Formatted Email or any string                             | `"glint@useglint.io"`                       |
...
:::tip
Glint supports standard Javascript regex syntax ([ECMA 262](https://www.ecma-international.org/publications-and-standards/standards/ecma-262/)), for quick reference of some of the available regex syntax, refer to the [JSON Schema docs](https://json-schema.org/understanding-json-schema/reference/regular_expressions.html)
:::
...
```json showLineNumbers
{
  ...
  // highlight-end
  "default": ["https://grafana.com", "https://useglint.io", "https://slack.com"]
...
It is possible to configure widget properties that expose completely new functionality in Glint, including new display options and loading data from external sources.
...
1. `type: object` - you provide the complete `json` spec into Glint.
...
| `agent` | `boolean` | Defines whether to use [Glint Agent](../../self-service-actions/webhook/glint-execution-agent/glint-execution-agent.md) for execution or not. <br></br> Can only be added if `type` is set to `WEBHOOK` | Either `true` or `false`    |
| `url`   | `string`  | Defines the webhook URL where Glint sends changelog events to via HTTP POST request. <br></br> Can be added only if `type` is set to `WEBHOOK`                                                        | `https://example.com`       |

For more information about Glint's changelog capabilities, refer to the [Self-Service Actions deep dive](../../self-service-actions/self-service-actions-deep-dive/self-service-actions-deep-dive.md) page.
