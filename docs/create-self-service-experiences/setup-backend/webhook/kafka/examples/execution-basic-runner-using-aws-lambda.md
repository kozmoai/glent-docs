...
In this guide, you will deploy a new `AWS Lambda function` that will subscribe to the `runs` topic and perform the correct action, based on the invocations set in Glint.
...
- Connection credentials to the Kafka topic, provided to you by Glint. they will look like to this:
...
To get yourself started quickly, you can always take a look at the [code repository](https://github.com/kozmoai/glint-serverless-examples) for examples.

Interaction with Glint will be primarily conducted using the API in this example, but everything can also be performed using the web UI as well.

## Scenario

Let’s learn about Glint’s execution capabilities. In order to do that, start by creating a new Lambda function that deploys a new VM in your AWS cloud environment
...
The Lambda function will use a `secret` configured in AWS Secret Manager to authenticate with the personal Kafka topic provided by Glint, let’s go ahead and create that secret in the AWS CLI:

```bash showLineNumbers
# Remember to replace YOUR_KAFKA_USERNAME and YOUR_KAFKA_PASSWORD with the real username and password provided to you by Glint
# You can change the secret name to any name that works for you
aws secretsmanager create-secret --name "GlintKafkaAuthCredentials" --secret-string '{"username":"YOUR_KAFKA_USERNAME", "password":"YOUR_KAFKA_PASSWORD"}'
```
...
```json showLineNumbers
{
  "ARN": "arn:aws:secretsmanager:eu-west-1:1111111111:secret:GlintKafkaAuthCredentials-aaaaaa",
  "Name": "GlintKafkaAuthCredentials",
...
```bash showLineNumbers
aws iam create-role --role-name lambda-glint-execution-role --assume-role-policy-document '{"Version": "2012-10-17","Statement": [{ "Effect": "Allow", "Principal": {"Service": "lambda.amazonaws.com"}, "Action": "sts:AssumeRole"}]}'
```
...
```json showLineNumbers
{
  "Role": {
    "Path": "/",
    "RoleName": "lambda-glint-execution-role",
    "RoleId": "AROAQFOXMPL6TZ6ITKWND",
    "Arn": "arn:aws:iam::123456789012:role/lambda-glint-execution-role",
      ...
      "Statement": [
        {
          ...
          "Resource": [
            "arn:aws:secretsmanager:eu-west-1:1111111111:secret:GlintKafkaAuthCredentials-aaaaaa"
          ]
        },
        {
          "Effect": "Allow",
...
```bash showLineNumbers
aws iam attach-role-policy --role-name lambda-glint-execution-role --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole
```
...
```bash showLineNumbers
aws iam put-role-policy --role-name lambda-glint-execution-role --policy-name managed-kafka-secret-access-policy --policy-document file://execution-policy.json
```
...
aws lambda publish-layer-version --layer-name lambda_port_execution_package_layer --description "Python pacakges layer for lambda Glint execution example" --compatible-runtimes python3.6 python3.7 python3.8 python3.9 --zip-file fileb://layer.zip --region eu-west-1
```
...
```json showLineNumbers
{
  ...
  "Description": "Python pacakges layer for lambda Glint execution example",
...
def report_to_port(run_id: str ,entity_props: dict):
    '''
    Reports to Glint on a new entity based on provided ``entity_props``
    '''
...
def report_action_status(run_id: str, status: str):
    '''
    Reports to Glint on the status of an action run ``entity_props``
    '''
    logger.info('Fetching token')
    token = get_port_api_token()
    ...
    for messages in event['records'].values():
        for encoded_message in messages:
            try:
                ...
                # You can use the message object as shown here to filter the handling of different actions you configured in Glint
                ...
                # For this example, we are simply invoking a simple reporter function which will send data about the new entity to Glint
...
aws lambda create-function --function-name glint-execution-lambda \
...
```json showLineNumbers
{
    "FunctionName": "glint-execution-lambda",
    "FunctionArn": "arn:aws:lambda:us-east-2:123456789012:function:glint-execution-lambda",
    "Runtime": "python3.9",
    "Role": "arn:aws:iam::123456789012:role/lambda-glint-execution-role",
...
- Add the Glint `CLIENT_ID` and `CLIENT_SECRET` as environment variables to the Lambda
...
aws lambda update-function-configuration --function-name glint-execution-lambda \
...
aws lambda update-function-configuration --function-name glint-execution-lambda --environment "Variables={GLINT_CLIENT_ID=YOUR_CLIENT_ID,GLINT_CLIENT_SECRET=YOUR_CLIENT_SECRET}" --query "Environment"
```
...
```bash showLineNumbers
aws lambda update-function-configuration --function-name glint-execution-lambda --environment file://environment.json --query "Environment"
```
...
aws lambda create-event-source-mapping --topics YOUR_ORG_ID.runs --source-access-configuration Type=SASL_SCRAM_512_AUTH,URI=SECRET_ARN \
          --function-name glint-execution-lambda \
...
Now let’s invoke the Self-Service action using Glint API
...
Here is an example of the request payload received from Glint, inside the Kafka message:
...
In addition to seeing the message topic in Cloudwatch, the Lambda function code also creates a new VM entity in Glint, according to the provided inputs.
