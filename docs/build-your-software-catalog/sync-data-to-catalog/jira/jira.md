...
Our Jira integration allows you to import `issues`, `boards`, `sprints` and `projects` from your Jira cloud account into Glint, according to your mapping and definition.

:::info Jira cloud only
This integration supports `Jira Cloud` at the moment. To integrate Glint with `Jira Server`, use [Glint's webhook integration](/build-your-software-catalog/custom-integration/webhook/examples/jira-server).
:::
...
| `integration.config.appHost`             | The host of the Glint Ocean app. Used to set up the integration endpoint as the target for webhooks created in Jira                         | https://my-ocean-integration.com | ❌       |
...
:::warning Realtime updates in Glint
...
| Parameter                        | Description                                                                                                                                                                                                                                                                             | Example                       | Required |
|----------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------|----------|
| `port_client_id`                 | Your Glint client ([How to get the credentials](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials)) id                                                                                                                           |                               | ✅        |
| `port_client_secret`             | Your Glint client ([How to get the credentials](https://docs.useglint.io/build-your-software-catalog/custom-integration/api/#find-your-glint-credentials)) secret                                                                                                                       |                               | ✅        |
...
| `initialize_port_resources`      | Default true, When set to true the integration will create default blueprints and the glint App config Mapping. Read more about [initializeGlintResources](https://ocean.useglint.io/develop-an-integration/integration-configuration/#initializeportresources---initialize-glint-resources) |                               | ❌        |
...
`createMissingRelatedEntities` - used to enable the creation of missing related entities in Glint. This is useful when you want to create an entity and its related entities in one call, or if you want to create an entity whose related entity does not exist yet.

`deleteDependentEntities` - used to enable deletion of dependent Glint entities. This is useful when you have two blueprints with a required relation, and the target entity in the relation should be deleted. In this scenario, the delete operation will fail if this parameter is set to `false`. If set to `true`, the source entity will be deleted as well.
:::

### Configuration structure

The integration configuration determines which resources will be queried from Jira, and which entities and properties will be created in Glint.
...
:::tip JQL Support
The Ocean Jira integration supports querying objects from the `issue` kind using [JQL](https://support.atlassian.com/jira-service-management-cloud/docs/use-advanced-search-with-jira-query-language-jql/), making it possible to specifically filter the issues that are queried from Jira and ingested to Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the Jira object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: project
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one Jira object to a Glint entity. Each value is a JQ query.
            identifier: .key
            title: .name
            blueprint: '"jiraProject"'
            properties:
...
{
  "id": 1,
  "self": "https://useglint.atlassian.net/rest/agile/1.0/board/1",
  "name": "GLINT board",
  "type": "scrum",
  "location": {
    "projectId": 10000,
    "displayName": "Glint (GLINT)",
    "projectName": "Glint",
    "projectKey": "GLINT",
    "projectTypeKey": "software",
    "avatarURI": "https://useglint.atlassian.net/rest/api/2/universal_avatar/view/type/project/avatar/10555?size=small",
    "name": "Glint (GLINT)"
  }
}
```
...
<details>
<summary>Board entity in Glint</summary>
...
<details>
<summary>Sprint entity in Glint</summary>
...
In this example you are going to create a webhook integration between [Jira](https://www.atlassian.com/software/jira) and Glint, which will ingest Jira issue entities.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map Jira issues to Glint`;
   4. Icon : `Jira`;
...
Done! any change you make to an issue (open, close, edit, etc.) will trigger a webhook event that Jira will send to the webhook URL provided by Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.
...
In this example you are going to use the provided Python script to fetch data from the Jira API and ingest it to Glint.
...
- `GLINT_URL` - the webhook URL generated by Glint after creating the webhook configuration;
...
This guide aims to cover how to connect a Jira `issue` to an existing service in Glint.
...
Labelling issues in Jira allows you to categorize and filter them. You can use labels to group issues that are related to a specific service in Glint. In this guide, we will add a label to tell us what service the issue is related to:
...
5. **Add a label** that represents the service the issue is related to, `glint-auth-service`. For this guide, let's assume there is a service entity identified by `auth-service` in your `Service` blueprint in Glint.

<img src='/img/guides/jiraAddLabelToIssue.png' width='60%' border='1px' />

:::note Control the label name
Since our Jira `issue` may already have several labels, we will need a mechanism to control how these labels will be related to our `Service` blueprint. A way to achieve this relation is to prefix the label name with the keyword `glint-`. We will then use JQ to select the labels that starts with this keyword. So, our example label will be named `glint-auth-service`, which will correspond to a Service entity identified by `auth-service` in Glint.
:::

### Create the service relation

Now that Glint is synced with our Jira resources, let's reflect the Jira Issue in our services to display the issue associated with a service.
...
1. Head back to the [Builder](https://app.useglint.io/dev-portal/data-model), choose the `Jira Issue` <GlintTooltip id="blueprint">blueprint</GlintTooltip>, and click on `New relation`:
...
Now that the <GlintTooltip id="blueprint">blueprints</GlintTooltip> are related, we need to assign the relevant Jira Issues to each of our services. This can be done by adding some mapping logic. Go to your [data sources page](https://app.useglint.io/dev-portal/data-sources), and click on your Jira integration:
      ...
      entity:
        mappings:
          ...
          relations:
            ...
            subtasks: .fields.subtasks | map(.key)
            // highlight-start
            service: .fields.labels | map(select(startswith("glint"))) | map(sub("glint-"; ""; "g")) | .[0]
            // highlight-end
```
...
The JQ below selects all labels that start with the keyword `glint`. It then removes "glint-" from each label, leaving only the part that comes after it. It then selects the first match, which is equivalent to the service in Glint.
...
By following these steps, you can seamlessly connect a Jira issue with an existing service blueprint in Glint using issue labels.
