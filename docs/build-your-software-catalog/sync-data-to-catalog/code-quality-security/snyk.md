...
Our Snyk integration allows you to import `organizations`, `targets`, `projects` and `issues` from your Snyk account into Glint, according to your mapping and definitions.
...
| `integration.config.appHost`    | The host of the Glint Ocean app. Used to set up the integration endpoint as the target for Webhooks created in Snyk | ❌       |
| `integration.secret.webhookSecret` | This is a password you create, that Snyk uses to sign webhook events to Glint                                       | ❌       |
...
helm upgrade --install my-snyk-integration kozmoai/glint-ocean \
...
variables:
  - group: glint-ocean-credentials # OCEAN__GLINT__CLIENT_ID, OCEAN__GLINT__CLIENT_SECRET, OCEAN__INTEGRATION__CONFIG__TOKEN
...
- script: |
    ...
    exit $?
  displayName: 'Ingest Synk Data into Glint'
...
The integration configuration determines which resources will be queried from Snyk, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the Snyk object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
  ...
  resources:
    - kind: project
      ...
      glint:
        # highlight-start
        entity:
          mappings: # Mappings between one Snyk object to a Glint entity. Each value is a JQ query.
            identifier: .id
            title: .attributes.name
            blueprint: '"snykProject"'
            properties:
...
In this example you are going to create a webhook integration between [Snyk](https://snyk.io/) and Glint, which will ingest Snyk code and infrastructure vulnerability entities into Glint.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map Snyk vulnerability to Glint`;
   4. Icon : `Snyk`;
...
curl -X POST \
     ...
     -d '{"url": "https://ingest.useglint.io/<YOUR_GLINT_WEBHOOK_KEY>", "secret": "WEBHOOK_SECRET"}' \
     https://api.snyk.io/v1/org/<YOUR_ORG_ID>/webhooks
```

:::

Done! Any vulnerability detected on your source code will trigger a webhook event to the webhook URL provided by Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.
</details>