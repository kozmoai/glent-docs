...
In this guide, we are going to present different methods to create an S3 bucket (e.g.), using Glint Self-Service Actions.
...
Each action shown in this example, creates a new Entity in Glint and updates action run info in order to keep track of the action status and its outcomes.
:::
...
In this [example](https://github.com/kozmoai/glint-action-runner-examples/tree/main/python/s3_bucket_creation/terraform_github_workflow/webhook), you will learn how to use GitHub pull-request, GitHub workflow, and Terraform to create an S3 bucket.
...
Check out this [example](https://github.com/kozmoai/glint-action-runner-examples/tree/main/python/s3_bucket_creation/github_action/webhook), to see how to implement S3 bucket creation, using GitHub actions from the public marketplace.
...
Here is an [example](https://github.com/kozmoai/glint-action-runner-examples/tree/main/python/s3_bucket_creation/aws_sdk/webhook), that provisions a simple backend in FastAPI (python), and uses [AWS SDK](https://aws.amazon.com/sdk-for-python/) to create a new S3 bucket.
