...
Assume you have an `array` property called `myArr` with the value `["this", "is", "glint"]`.
...
```json
{
  "kubernetes.io/metadata.name": "glint-k8s-exporter",
  "name": "glint-k8s-exporter"
}
```
...
The result will be a property that displays `glint-k8s-exporter`.
...
In Glint's AWS exporter, you can find the following array:
