...
In the following example you will export your Kubernetes `Replica Sets` and `Pods` to Glint, you may use the following Glint blueprint definitions, and exporter configuration:
...
<details>
<summary> Glint K8s Exporter configuration </summary>
...
After creating the blueprints, open your integration in the [data sources](https://app.useglint.io/dev-portal/data-sources) page, apply the provided configuration and click on the `Save & Resync` button.

Done! The exporter will begin creating and updating objects from your Kubernetes cluster as Glint entities shortly.

For instance, you can see a `Deployment Config` and its `Pods` in a single Glint entity page:
