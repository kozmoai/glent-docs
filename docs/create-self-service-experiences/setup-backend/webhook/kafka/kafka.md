...
Glint manages a Kafka Topic per customer that publishes the execution run requests.
...
![Glint Kafka Architecture](/img/self-service-actions/glintKafkaArchitecture.jpg)

The steps shown in the image above are as follows:

1. Glint publishes an invoked `Action` or `Change` message to Kafka;
...
- A group name that matches your username provided by Glint
