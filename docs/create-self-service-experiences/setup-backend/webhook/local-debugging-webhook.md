...
In this guide, we will show you how to debug Webhook Self-Service Actions that are sent from Glint locally.

This example contains the initial steps to set up a standard Self-Service Action use case, following it shows how to locally debug the payload sent by Glint through the webhook [invocation method](/create-self-service-experiences/setup-backend/#json-structure).
...
In this example, interaction with Glint will be primarily conducted using the API, but can also be done using the web UI.

## Scenario

You want to provision new VMs using Glint's `CREATE` Self-Service Actions.

In this example you will:

- Create a new `VM` Blueprint
- Add a `CREATE` action to the Blueprint
- Use a local web-server to debug the webhook requests that Glint sends every time a developer asks for a new VM.
...
app.post("/webhooks", (request, response) => {
  // This part is used to verify that the webhook message was sent by Glint
  const signed = createHmac("sha256", "<CLIENT_SECRET>")
    .update(
      `${request.headers["x-glint-timestamp"]}.${JSON.stringify(request.body)}`
    )
    .digest("base64");

  if (signed !== request.headers["x-glint-signature"].split(",")[1]) {
...
app.listen(glint, () => {
  console.log(`Example app listening on glint ${glint}`);
});
```
...
Login to glint and go to the VM page and trigger the action via the **Create VM** action button:
