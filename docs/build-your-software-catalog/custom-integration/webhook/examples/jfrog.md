...
In this example you are going to create a webhook integration between your JFrog Server and Glint. The integration will facilitate the ingestion of JFrog artifact, Docker tag and build entities into Glint.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map JFrog repositories and builds to Glint`;
   4. Icon : `JfrogXray`;
...
6. Input the following details:
   1. `Name` - use a meaningful name such as **Glint-Artifact**;
   2. `Description` - enter a description for the webhook;
   3. `URL` - enter the value of the `url` key you received after creating the webhook configuration in Glint;
...
8. Create two more webhooks using the details:
   1. For builds:
      ...
      - `URL` - enter the value of the `url` key you received after creating the webhook configuration in Glint;
   2. For Docker tags:
      - Name: **Glint-Docker-Tag**;
...
Done! Any artifact you publish, build you trigger, or artifact you upload will trigger a webhook event that JFrog will send to the webhook URL provided by Glint. Glint will parse the events according to the mapping and update the catalog entities accordingly.
...
In this example you are going to use the provided Python script to fetch data from the JFrog Server API and ingest it to Glint.
...
Use the following Python script to ingest historical JFrog builds and repositories into glint:
...
GLINT_CLIENT_ID = os.getenv("GLINT_CLIENT_ID")
GLINT_CLIENT_SECRET = os.getenv("GLINT_CLIENT_SECRET")
...
def add_entity_to_port(blueprint_id, entity_object, transform_function):
    """A function to create the passed entity in Glint

    Params
    --------------
    blueprint_id: str
        The blueprint id to create the entity in Glint

    entity_object: dict
        The entity to add in your Glint catalog

    transform_function: function
        A function to transform the entity object to the Glint entity object
    ...
    """
    logger.info(f"Adding entity to Glint: {entity_object}")
    entity_payload = transform_function(entity_object)
    response = requests.post(
        (
            f"{GLINT_API_URL}/blueprints/"
...
if __name__ == "__main__":
    logger.info("Starting Glint integration")
...
Done! you can now import historical builds and repositories from JFrog into Glint. Glint will parse the builds and repository according to the mapping and update the catalog entities accordingly.
