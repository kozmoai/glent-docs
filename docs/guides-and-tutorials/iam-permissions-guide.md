...
Developers need access to different cloud resources on a daily basis. With all of your most important AWS resources being exported to Glint, you can create an experience that allows your developers to request IAM permissions directly from Glint.
...
In this step-by-step guide, we will create Glint blueprints and actions, which will allow you to request and revoke IAM permissions for different AWS resources using Glint. You will also be able to keep track of which permissions were requested, and who requested them.

## Prerequisites
- Prepare your Glint organization's `Client ID` and `Client Secret`. To find you Glint credentials, click [here](/docs/build-your-software-catalog/custom-integration/api/api.md#find-your-glint-credentials).
- In your AWS console, [create an IAM user](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html) called `glint-iam-management-user` with the following IAM permissions policy:
    <details>
...
- [Create access credentials](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html) for the IAM user `glint-iam-management-user` (`AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`).

- In your GitHub organization, create a new repository called `glint-iam-permissions`. You will use this repository to maintain your GitHub workflows, and other dependency files.

- Install Glint's GitHub app by clicking [here](https://github.com/apps/useglint-io/installations/new). Make sure to give the Glint GitHub app permissions for the `glint-iam-permissions` repository.

- Create the following secrets as [GitHub Actions secrets](https://docs.github.com/en/actions/security-guides/using-secrets-in-github-actions) in the `glint-iam-permissions` repository:
    ...
    - `AWS_ACCESS_KEY_ID` - The `AWS_ACCESS_KEY_ID` generated for the `glint-iam-management-user` IAM user.
    - `AWS_SECRET_ACCESS_KEY` - The `AWS_SECRET_ACCESS_KEY` generated for the `glint-iam-management-user` IAM user.
    - `AWS_REGION` - Your primary AWS region (you can set this to `us-east-1` if you are unsure).

## Data Model
For this guide, we will be creating <GlintTooltip id="blueprint">blueprints</GlintTooltip> responsible for managing and keeping track of your different AWS resources, and your developers' IAM permission requests.
...
<details>
    ...
    The entities of this blueprint will represent the permissions which were created and managed using Glint.
...
## Actions
We want to be able to provision and revoke permissions for AWS resources from Glint. To do so, we will need to create some <GlintTooltip id="action">actions</GlintTooltip> in our Glint organization, and set up some action backends.

### Actions backend - GitHub Workflows
As mentioned in the [prerequisites](#prerequisites), in this guide we will be using [GitHub actions](https://docs.github.com/en/actions) as a backend for our Glint actions. To do this, we will create 2 GitHub workflow files, and 2 JSON files which will be used as templates for developer IAM permissions. 

The files mentioned above should be created in the `glint-iam-permissions` repository you set up in the prerequisites section.

In the workflow files, we will be using the [AWS CLI](https://aws.amazon.com/cli/) to interact with AWS, in order to create and delete the relevant resources when managing the IAM permissions using Glint.

Create the following files your `glint-iam-permissions` repository, in the correct path as it appears in the filename:
  ...
  workflow_dispatch:
    inputs:
      port_payload:
        type: string
        required: true
        description: The Glint Payload for triggering this action                
...
jobs:
  create-iam-permissions:
    ...
    steps:
      ...
      - name: "Report permission to Glint 🚢"
...
jobs:
  delete-permissions:
    ...
    steps:
      ...
      - name: "Delete permission from Glint 🚢"
        uses: kozmoai/glint-github-action@v1
        with:
          clientId: ${{ secrets.GLINT_CLIENT_ID }}
          clientSecret: ${{ secrets.GLINT_CLIENT_SECRET }}
...
After creating our backend in GitHub, we need to create the Glint actions to trigger the workflows we created.
...
Let's create the Glint actions to tirgger the workflows we just created:
<details>
    <summary>`Request permissions` Glint action</summary>
...
<details>
    <summary>`Revoke permissions` Glint action</summary>
...
## Manage permissions using Glint
...
Managing the AWS resources we want to provision permissions for will be done using Glint entities. Navigate to the [AWS Resources](https://app.useglint.io/aws_resources) catalog page to create some example entities.

:::info Simple permissions to get started
For this guide's simplicity, we will be creating AWS resource entities manually. This can also be done using Glint's [AWS Exporter](/docs/build-your-software-catalog/sync-data-to-catalog/cloud-providers/aws/aws.md). Go to the [Next Steps](#next-steps) section to read more.
:::
...
Managing the IAM permissions we want to allow our developer to provision will be done using Glint entities. Navigate to the [IAM Permissions](https://app.useglint.io/iam_permissions) catalog page to create some example entities.
...
Now that we finished setting up our Glint environment, and our action backends, we are ready to manage IAM permissions for our AWS resources!

#### Provision permissions
Let's start by creating new temporary permissions for our S3 bucket `my-s3-bucket`, using Glint. Navigate to the bucket's [entity page](https://app.useglint.io/aws_resourceEntity?identifier=arn%3Aaws%3As3%3A%3A%3Amy-s3-bucket).
...
The actoin will also create a new `Provisioned Permissions` entity which you can see in the [Provisioned Permissions](https://app.useglint.io/provisioned_permissions) catalog page.

Copy the sign-in URL and paste it to your browser's URL bar. Click the `Switch Role` button. 

You are now signed in to your new role, which has permissions as defined in the Glint action! 🥳
...
Navigate to the [Provisioned Permissions](https://app.useglint.io/provisioned_permissions) catalog page -> Click on the new `Permission-XXXXXXXX` entity.
...
Check out the [Provisioned Permissions](https://app.useglint.io/provisioned_permissions) catalog page to view all active permissions. There, you can see important information like the permissions' requesters, the IAM policies, sign-in URLs and more.
<details>
...
## Summary 
That's it! You are all set up to manage IAM permissions for your different AWS resources using Glint!🚀
...
## Next Steps
- **Install Glint's [AWS exporter](/docs/build-your-software-catalog/sync-data-to-catalog/cloud-providers/aws/aws.md)** - You can use Glint's AWS exporter to automatically populates your software catalog from your AWS environement. You can use the AWS exporter to populate your `AWS Resources` blueprints with different AWS resources.
- **Enforce [manual approval](/docs/create-self-service-experiences/set-self-service-actions-rbac/set-self-service-actions-rbac.md#configure-manual-approval-for-actions) for your Glint actions** - To have control over who is provisioning and revoking permissions, you can set up manual approval for your actions. This will enable you to provide a request-approve flow for provisioning and revoking permissions using Glint.
<!-- TODO: Remove the `coming soon` when automiations comes out -->
- ***Coming soon* ⏱️: Temporary permissions using [Automations](https://roadmap.useglint.io/ideas/p/automation) and the `Timer` property** - With the automations feature, you will be able to automatically trigger actions using events from the catalog. You can use the `Timer Expired` event to trigger the `Revoke permissions` action and create a temporary permissions experience.
