...
In this tutorial, we will export pull requests from Bitbucket and create matching Glint Entities!

1. Create a `pullRequest` Blueprint and `glint-app-config.yml` configuration file.

To export your Bitbucket `Pull Requests` to Glint, you can use the following Glint Blueprints definitions, and `glint-app-config.yml`:
...
- See how to apply `glint-app-config.yml` either on the repository level or globally [here](./configuration.md).
...
- If you have already synced your [repositories](./exporting-repositories.md), you can use the following blueprint and `glint-app-config.yml` to create and populate a relation from pull request to repository (`microservice` blueprint):
...
Now you can view and query all of your Pull Requests as Glint Entities!
