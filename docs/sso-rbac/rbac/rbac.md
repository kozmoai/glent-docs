...
Glint's RBAC mechanism makes it possible to assign permissions to specific users and teams, as well as configure custom roles tailored to the needs of the different personas in Glint.

## Assigning permissions

In Glint, you can assign permissions by using [roles](#roles), [team ownership](/build-your-software-catalog/set-catalog-rbac/examples.md#team-ownership-examples) and [users](/build-your-software-catalog/set-catalog-rbac/examples.md#user-examples).
...
In Glint, you can control and manage all your users and teams, in one place.
...
- Glint's [Terraform provider](#terraform-provider)
- The [Glint API](#glint-api)
...
**Note:** the following limitations do not apply to teams created manually inside Glint.
...
You can perform the aforementioned actions via the [Terraform provider](https://registry.terraform.io/providers/kozmoai/kozmoai/latest).  
Here is a basic example of a `main.tf` file that defines a team with 3 users:

```bash showLineNumbers
resource "glint_team" "example" {
  name        = "example"
  description = "example"
  users = [
    "user1@test.com",
...
You can browse the `team` schema [here](https://registry.terraform.io/providers/kozmoai/kozmoai/latest/docs/resources/glint_team).

### Glint API

The API allows you to manage [teams](https://api.useglint.io/static/index.html#/Teams) and [users](https://api.useglint.io/static/index.html#/Users).
...
- We support the manual creation of teams on Glint, as well as integrating with identity providers, such as [Okta](../sso-providers/okta.md) and [AzureAD](../sso-providers/azure-ad.md), to import existing teams.
- When users log in to Glint, their groups will be pulled automatically from their identity provider, and the allowed team values will be updated accordingly.
