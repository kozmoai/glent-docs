...
This guide takes 7 minutes to complete, and aims to demonstrate the power of self-service actions in Glint.
...
As part of the onboarding process, you should already have an action named `Scaffold a new service` in your [self-service tab](https://app.useglint.io/self-serve).  
...
2. Each action in Glint is directly tied to a <GlintTooltip id="blueprint">blueprint</GlintTooltip>. Since we are creating a repository, let's use the `Service` blueprint that was created for us as part of the [onboarding](/quickstart) process. Choose it from the dropdown.
...
- We set the type to `Text` since this is a name, but note all of the different types of input that Glint allows.
...
Now we'll define the backend of the action. Glint supports multiple invocation types, one of them should be selected for you depending on the Git provider you selected in the beginning of the onboarding process.
...
- Name the workflow `glint-create-repo.yml`.
...
:::info Important
If the Github organization which will house your workflow is not the same as the one you'll create the new repository in, install Glint's [Github app](https://github.com/apps/useglint-io) in the other organization as well.
:::
...
3. Now let's create the workflow file that contains our logic. Under `.github/workflows`, create a new file named `glint-create-repo.yml` and use the following snippet as its content (remember to change `<YOUR-ORG-NAME>` on line 19 to your GitHub organization name):

<details>
<summary><b>Github workflow (click to expand)</b></summary>

```yaml showLineNumbers title="glint-create-repo.yml"
...
# highlight-next-line
    ...
    steps:
      - uses: kozmoai/cookiecutter-gha@v1.1.1
        id: scaff
        with:
...
:::tip
This workflow uses Glint's [cookiecutter Github action](https://github.com/kozmoai/cookiecutter-gha) to scaffold the new repository.
:::
...
:::tip
Now that you have both the new GitLab project and its respective trigger token, you can go to the [define backend type](#define-backend-type) section and complete the action configuration in Glint.
:::
  ...
  script:
    - |
      ...
      default_context:
        ...
        email: "${EMAIL}"
        project_short_description: "Project scaffolded by Glint"
...
create-entity:
  ...
  script:
    - |
      echo "Creating Glint entity to match new repository"
      curl -X POST \
          -H 'Content-Type: application/json' \
          -H "Authorization: Bearer $ACCESS_TOKEN" \
...
:::tip
Now that you have the `JOB_TOKEN` value, you can go to the [define backend type](#define-backend-type) section and complete the action configuration in Glint.
:::
                ...
                script {
                    def logs_report_response = sh(script: """
                        curl -X POST \
                          -H "Content-Type: application/json" \
                          -H "Authorization: Bearer ${GLINT_ACCESS_TOKEN}" \
                          -d '{"message": "Creating ${SERVICE_NAME} Microservice Glint entity..."}' \
                             "https://api.useglint.io/v1/actions/runs/${GLINT_RUN_ID}/logs"
                    """, returnStdout: true)
...
Congratulations! You can now create services easily from Glint 💪🏽
