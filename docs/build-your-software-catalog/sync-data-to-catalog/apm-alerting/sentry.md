...
Our Sentry integration allows you to import `projects`, `issues`, `project-tag` and `issue-tag` from your Sentry cloud account into Glint, according to your mapping and definition.
...
- Map your monitored projects and issues into Glint.
...
helm upgrade --install my-sentry-integration kozmoai/glint-ocean \
...
:::warning
If you want the integration to update Glint in real time you should use the [Real Time & Always On](?installation-methods=real-time-always-on#installation) installation option
:::
...
The integration uses polling to pull the configuration from Glint every minute and check it for changes. If there is a change, a resync will occur.
...
The integration configuration determines which resources will be queried from Sentry, and which entities and properties will be created in Glint.
...
- The `glint`, `entity` and the `mappings` keys are used to map the Sentry object fields to Glint entities. To create multiple mappings of the same kind, you can add another item in the `resources` array;
...
<details>
<summary> Project environment entity in Glint</summary>
...
In this example you are going to create a webhook integration between [Sentry](https://sentry.io) and Glint, which will ingest issues entities.
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map Sentry Issues to Glint`;
   4. Icon : `Sentry`;
...
So when following this example, please first create the webhook configuration in Glint. Use the webhook URL from the response and create the webhook in Sentry.
After getting the secret from Sentry, you can go back to Glint and update the [webhook configuration](/build-your-software-catalog/custom-integration/webhook/?operation=ui#configuring-webhook-endpoints) with the secret.
:::
...
6. Input the following details:
   ...
   5. `Webhooks` - Under this section, enable the issues checkbox to allow Sentry to report issue events to Glint;
7. Click **Save Changes** at the bottom of the page.

:::tip
Now that the webhook is created, you can take the secret value generated by Sentry and use it to update the `security` object in your Glint webhook configuration
:::
...
1. **Basic details** tab - fill the following details:
   ...
   3. Description : `A webhook configuration to map Sentry Comments to Glint`;
   4. Icon : `Sentry`;
2. **Integration configuration** tab - fill the following JQ mapping:
...
Done! any issue and comment in Sentry will trigger a webhook event. Glint will parse the events according to the mapping and update the catalog entities accordingly.
...
The combination of the sample payload and the webhook configuration generates the following Glint `sentryIssue` entity:
...
In addition, the following Glint `sentryComment` entity will be generated:
